...
Doc version: v0.7.0-complete-change-picture-reconciliation
...
| `сверь айтемы`, `сверь айтемы с документацией`, `проверь айтемы по репозиторию`, `reconcile planning items`, `reconcile items` | `reconcile planning items` | Reconcile selected working, local or unprocessed Planning Items by reviewing the complete current picture they change, not only isolated item rows or changed fragments. | Use the clearly active item set or a same-message attached item source. Ask which item source to use only when it is missing or ambiguous; do not silently select an older local file. Choose the primary complete picture from what the items actually change. | Targeted/full by affected picture: selected item source → primary complete picture candidates → complete current owner artifacts → source-linked origins and supporting consistency sources → dependent or neighbouring pictures. | This UCM, `planning/documentation/application-planning/application-planning-drafting-workflow.md`, the selected item source, complete current primary-picture owners and relevant supporting owner files. | Read-only reconciliation review for each affected workflow, active Planning Draft/Full Picture, Functional Workflow, branch, detail, model or other coherent primary object: complete before/after picture, change map, preserved purpose/boundaries, supporting principles/terminology/templates, conflicts, unresolved choices and cross-picture consistency when several primary pictures change. No file edit, item-register update, archive, commit or push. |

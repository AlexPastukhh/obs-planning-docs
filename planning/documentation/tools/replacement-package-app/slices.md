...
**Token-binding outcome extension:** successful Apply remains its own success notification. If chatContextToken binding is not ready at the Review delivery cutoff, emit a separate “ReviewDiff not sent — chat binding” notification; if resolution succeeds later, emit a separate “Review chat bound” notification that explicitly states the preceding ReviewDiff was not automatically sent. These notifications do not rewrite ChangeSet operation truth.

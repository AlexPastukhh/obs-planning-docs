...
**Current implementation after this correction:** persistent Review-chat binding with two entry paths (manual selection and optional unique exact `OBS-ACTION.chatTabTitle` hint after successful Apply), both converging on the same binding service; automatic/manual queueing, duplicate-tab claim serialization, exact artifact verification, task-specific `.diff` attachment identity through the shared attachment primitive, `Preparing` only after upload-ready verification, composer-clean enforcement through the MAIN-world pre-click guard, actual click as the possible-Send boundary, post-baseline `.diff` attachment-surface confirmation from the complete user-turn container for `Sent`, configurable guarded Send attempts while the same attachment remains prepared, and immutable terminal outcomes after external uncertainty. Existing binding wins over an action hint; zero/ambiguous title matches never guess or fail Apply and retain manual fallback. Live Edge practical acceptance remains required.

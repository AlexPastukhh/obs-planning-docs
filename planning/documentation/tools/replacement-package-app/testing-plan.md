...
- failed token-bearing Apply proof requires `ApplyFailed` to remove that request from the agent-facing pending snapshot and ignore late in-flight results; an exact same package/ChangeSet/token retry may reopen it, while a different package/ChangeSet remains rejected.

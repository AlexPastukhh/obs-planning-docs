...
|---|---|
| `SL-RPKG-01` Apply Replacement Work | `CoreTests`: legacy package/action/ownership/source behavior plus Git-backed `Ready → AppliedUncommitted → CommittedUnpublished`: worktree-only mutation, durable Apply-journal exact prior/intended bytes, same-package idempotency, Apply crash recovery, Unicode/raw dirty-path identity, Commit package-only staging including explicit ignored adds, exact authoritative trailer-block parsing isolated from ChangeSet label/body text, exact trailers/parent, journal-only staged retry, failed-Commit retry without reapplying, post-commit pre-ledger crash recovery, repeat Commit/same-package Apply satisfaction, foreign moved-head refusal, unrelated-dirt refusal and legacy Review/Finalize fencing. |

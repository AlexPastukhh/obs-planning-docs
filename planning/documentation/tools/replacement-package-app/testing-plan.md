...
- ordinary ChatGPT conversation discovery, including exact-title action-assisted binding and duplicate-title/manual fallback;
...
| `SL-RPKG-06` Deliver Current Change | manual/action-assisted binding convergence; exact-title unique/no-match/ambiguous/existing-binding rules; exact delivery integration/dedupe/uncertainty + task-specific attachment identity + post-preparation composer guard + actual-click possible-Send boundary + per-task retry interval | real manual binding plus action-title unique auto-bind/manual fallback/existing-binding priority; small/large `.diff` attachment delivery with ChatGPT foreground and non-foreground; upload-ready/task-specific prepared-filename verification; post-Send `.diff` attachment-surface DOM inspection; unrelated composer/user-turn negatives; configured guarded MAIN-world Send retries; send/uncertainty outcome |
...
- optional `OBS-ACTION.chatTabTitle` parsing is backward compatible; unique exact current title on an unbound ChangeSet uses the normal persisted Review-chat binding and queues the current ReviewDiff; an existing binding wins; zero/ambiguous matches preserve successful Apply and produce manual-fallback warning without a guessed binding.

...
Browser agent lifecycle is intentionally single-owner. The manifest does not declare ChatGPT `content_scripts`; the extension service worker is the only injector and reconciles already-open/newly-navigated ordinary ChatGPT tabs through `ensureContentScript(...)`. One `runtimeGeneration` is kept in `chrome.storage.session`, which survives ordinary MV3 service-worker suspension/restart but rotates when the extension runtime is reloaded/updated. Each injected document agent has an `agentInstanceId`; background ping/task/control and payload-stream envelopes reject stale generations/instances. Reinjecting inside the same live extension context disposes the prior agent and heartbeat timer. If the extension context itself is invalidated, the old agent deactivates and clears its timer instead of continuing an exception loop. This lifecycle fence is extension-internal and does not redefine Java task identity or protocol `2`.
...
→ optional turn-local `.diff` attachment surface = stronger proof only

...
Main.java                       fixed CLI + JAR entry
            ↓
Core.java                       package/state/review/finalize mechanics owner
            ↓
...
## 2. Repository And Archive Inputs

The application has a configured `repositoryRoot`. Core verifies a Git work tree and derives raw origin identity from `git config --get remote.origin.url`.

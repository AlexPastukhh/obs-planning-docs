...
For `OBS-ACTION.chatTabTitle`, Core's Prepare phase applies `ReviewChatTitleMatcher` using the persisted ignored-character set, freezes a unique current `conversationKey` when one exists and records no/ambiguous conditions as Output warnings. When that unique destination differs from an existing binding, the Swing host asks before mutation whether to keep the current binding, rebind after successful Apply, or cancel. `ChatBridgeService.assertRebindSafe(...)` continues to protect that legacy/manual path. `OBS-ACTION.chatContextToken` is deliberately stronger: the originating explicit Helper `Bind + ...` action is itself bind/rebind authorization, so a unique token resolution writes the captured conversation as the persisted binding immediately, independent of repository Apply outcome and without another confirmation. Safely cancellable old Review tasks are cancelled on destination change; any task already past composer preparation keeps its frozen original conversation while the persisted binding changes for subsequent work. Automatic ReviewDiff queueing still occurs only after the canonical current ReviewDiff is persisted. Browser delivery failure is warning/downstream state and never rolls back Apply/Refresh Review or changes Finalize authority.

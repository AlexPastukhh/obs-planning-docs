...
When Java executes an `OBS-ACTION` carrying `chatContextToken`, it persists a pending lookup and increments the chat-context request revision. Extension background keeps one authenticated bounded long-poll on `/v1/chat-context/wait`; a new lookup wakes that request immediately, so initial token resolution no longer waits for the next inventory cycle. Background then asks all live `chatgpt.com` agents with `OBS_CHAT_CONTEXT_LOOKUP`; agents answer only from their own session store, so navigation after the capture cannot reinterpret an old token as the new current conversation. Background aggregates answers and POSTs `/v1/chat-context/result`. Duplicate answers for one conversation are equivalent; different conversation keys for one token are a conflict. Empty results remain pending in the extension's remembered request set. A bounded-wait timeout with an unchanged revision only renews the long-poll and does not re-query agents; unresolved tokens retry when Java changes the authoritative pending revision or on relevant tab lifecycle events. Inventory remains only conversation/task reconciliation and no longer carries lookup work.

This lookup is internal destination resolution, not an External Interaction. Apply and lookup run asynchronously. A unique token result is explicit binding authority and is applied immediately: unbound → bind, same destination → confirm/refresh, different destination → rebind. This happens regardless of repository Apply success/failure; repository failure does not cancel the pending lookup or roll back a token-resolved binding. At the successful Apply ReviewDiff cutoff Java queues the current ReviewDiff only when the token has already resolved to its persisted binding. Pending/conflicted lookup skips that ReviewDiff and records the skipped delivery separately. Late resolution still binds/rebinds immediately for subsequent deliveries but never queues the already-skipped ReviewDiff. A token remains package/ChangeSet-scoped and cannot be carried into another request.

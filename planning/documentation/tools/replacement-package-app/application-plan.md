...
Manual Review-chat selection remains an explicit binding method and persists with the ChangeSet. `OBS-ACTION/1` may additionally carry either legacy `chatTabTitle` or an invocation-scoped `chatContextToken`. Token presence suppresses title matching for that action. The bridge resolves a token by asking live tab agents for the capture stored in that tab's `sessionStorage`; this is destination discovery for the same persisted binding service, not a second delivery subsystem. Repository Apply does not wait for the lookup: the successful-Apply/current-ReviewDiff point is the automatic-delivery cutoff. Pending/conflict/different-binding at that point skips only that automatic ReviewDiff delivery and emits a separate notification; late success may persist a missing binding for future deliveries and emits its own notification without retroactive Send.
...
- `REQ-RPKG-29` — optional invocation-scoped `OBS-ACTION.chatContextToken` resolves asynchronously through live ChatGPT tab agents from captured `sessionStorage`; token lookup never blocks repository Apply, token presence takes precedence over legacy title matching, pending/conflict/different-binding at the successful Apply ReviewDiff cutoff skips only that automatic delivery, and late binding is separately notified and never retro-sends the skipped ReviewDiff.

...
Manual Review-chat selection remains an explicit binding method and persists with the ChangeSet. `OBS-ACTION/1` may additionally carry either legacy `chatTabTitle` or an invocation-scoped `chatContextToken`. Token presence suppresses title matching for that action. The explicit Helper `Bind + ...` invocation makes the token direct bind/rebind authority: after one unique capture resolves, the same persisted binding service immediately writes that conversation whether the ChangeSet was unbound, already bound there or bound elsewhere. Repository Apply does not wait for the lookup and does not own/cancel its binding result. The successful-Apply/current-ReviewDiff point is only the automatic-delivery cutoff: Pending/Conflict there skips that ReviewDiff and emits a separate notification; late success still binds/rebinds for future deliveries without retroactive Send.
...
There is no small/large ReviewDiff threshold: exact attachment delivery is the single path. ReviewDiff and Snapshot use the same generic attachment/optional-Send module; only artifact content type, Snapshot timeout staging, and whether `autoSend` is selected differ. The existing persisted retry setting is frozen into every auto-send task. Bridge protocol `5` validates the generic claimed-task contract before payload/composer mutation; stale app/extension pairs and malformed send fields are `FailedBeforeSend`, not post-Send uncertainty.
...
- `REQ-RPKG-29` — optional invocation-scoped `OBS-ACTION.chatContextToken` resolves asynchronously through live ChatGPT tab agents from captured `sessionStorage`; token presence is explicit bind/rebind authority and takes precedence over legacy title matching; unique resolution immediately persists the captured conversation regardless of repository Apply outcome; pending/conflict at the successful Apply ReviewDiff cutoff skips only that automatic delivery; late binding/rebinding is separately notified and never retro-sends the skipped ReviewDiff.
...
| `P-RPKG-BRIDGE-VERSION-SKEW` | Live attachment test prepared the exact `.diff` but then surfaced `UnknownAfterSend · Invalid ReviewDiff send retry interval`, showing a deterministic Java↔extension contract mismatch can be discovered after the possible-Send boundary. | Explicit bridge protocol v5 + complete claimed-task preflight before payload/composer mutation; actionable mismatch is `FailedBeforeSend`; retry interval is carried prevalidated across `SendClicked`. | P0 practical correction / SL-06 |

...
| [`SCN-RPKG-PROVIDE-REPOSITORY-CONTEXT`](scenarios/SCN-RPKG-PROVIDE-REPOSITORY-CONTEXT.md) | Produce exact portable repository context and optionally make it ready in the intended ChatGPT conversation without changing repository work. | A valid Repository Snapshot exists; optional attachment is ready in the intended conversation and is never auto-sent. | current export/attach exists; destination-first Export + Attach selects/fixes the conversation before export while preserving independent export success |
...
For Repository Snapshot `Export + Attach`, destination choice is operation input rather than a late post-export step. The dialog exposes `Export only` and `Export + Attach`; the latter requires an explicitly selected currently open ordinary conversation before export begins and freezes its `conversationKey`. The snapshot ZIP must become successfully authoritative before an External Interaction is created. Snapshot attachment never resolves or substitutes a later/current chat and never reads/writes the ChangeSet Review-chat binding. The host deliberately does not add a fresh-inventory handshake just to detect tab closure during export: known-missing destination at enqueue still rejects immediately, while any queued Snapshot interaction has one absolute 10-minute confirmation deadline from task creation. Expiry before confirmed external preparation is terminal `Cancelled`; expiry after preparation began is terminal `PreparedUnsent`, preserving the possibility that the attachment remains in the composer.
...
- after a possible Send, once the exact prepared attachment has left the composer, a new user turn after the preparation baseline is sufficient to confirm `Sent`; a turn-local `.diff` attachment/file surface is stronger optional proof when ChatGPT exposes it; if the attachment disappears after a possible Send and no such post-baseline user turn is observed, result is `UnknownAfterSend` and automation stops.
...
- `REQ-RPKG-08` — snapshot attachment never auto-sends the ChatGPT message; `Export + Attach` selects and freezes one exact conversation before export starts, creates the attach interaction only after successful export, never substitutes a later/current chat, and never changes Review-chat binding state. A queued Snapshot interaction cannot remain actionable indefinitely: one fixed 10-minute confirmation deadline yields `Cancelled` before confirmed preparation or `PreparedUnsent` after preparation began.

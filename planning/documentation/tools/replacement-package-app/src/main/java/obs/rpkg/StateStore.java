...
final class StateStore {
    ...
    @SuppressWarnings("unchecked") Core.Settings getSettings(){
        ...
        String ignored=Core.str(m.get("reviewChatTitleIgnoredCharacters"));if(ignored==null)ignored="";if(ignored.codePointCount(0,ignored.length())>128||ignored.indexOf('\n')>=0||ignored.indexOf('\r')>=0)throw new Core.ObsException(Core.STATE_DIVERGED,"Invalid settings.json reviewChatTitleIgnoredCharacters.");
        ...
        if(version>=2){
            ...
            return new Core.Settings(List.copyOf(repos),blankToNull(Core.str(m.get("selectedRepositoryId"))),blankToNull(Core.str(m.get("selectedChangeSetId"))),handling,retry,ignored);
        }
        String legacy=Core.str(m.get("repositoryRoot"));
        if(legacy==null||legacy.isBlank())return new Core.Settings(List.of(),null,null,handling,retry,ignored);
        ...
        return new Core.Settings(List.of(new Core.RepositoryConfig(id,displayName(legacy),legacy,"")),id,null,handling,retry,ignored);
    }

    void saveSettings(Core.Settings s){
        Map<String,Object>m=new LinkedHashMap<>();m.put("schemaVersion",4);List<Object> repos=new ArrayList<>();

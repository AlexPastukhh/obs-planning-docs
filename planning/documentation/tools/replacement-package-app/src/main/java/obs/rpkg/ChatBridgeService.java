...
final class ChatBridgeService {
    ...
    synchronized PayloadSource verifiedPayloadSource(String taskId,String ticket){
        ...
        return new PayloadSource(p,actualSize,actualSha,"snapshot".equals(t.kind)?"application/zip":"text/plain; charset=utf-8",t.fileName);
    }
    ...
    synchronized void result(String taskId,int tabId,String conversationKey,String status,String message){
        ...
        if("snapshot".equals(t.kind)){
            boolean ok=("Claimed".equals(t.status)&&"FailedBeforeSend".equals(status))||("Preparing".equals(t.status)&&Set.of("Attached","PreparedUnsent").contains(status));if(!ok)throw fail("Invalid snapshot task transition "+t.status+" → "+status+".");
        }else if("reviewDiff".equals(t.kind)){
            boolean ok=("Claimed".equals(t.status)&&"FailedBeforeSend".equals(status))||("Preparing".equals(t.status)&&"PreparedUnsent".equals(status))||("SendClicked".equals(t.status)&&Set.of("Sent","UnknownAfterSend").contains(status));if(!ok)throw fail("Invalid ReviewDiff task transition "+t.status+" → "+status+".");
    ...
    private static final class Task {
        String taskId,kind,changeSetId,reviewAttemptId,conversationKey,conversationTitle,artifactPath,artifactSha256,fileName,status,message,createdAt,updatedAt,leaseUntil,payloadTicket,ticketExpiresAt;long artifactSize;boolean autoSend,autoGenerated;Integer claimedTabId;
        Map<String,Object> json(){Map<String,Object> m=new LinkedHashMap<>();m.put("schemaVersion",1);m.put("taskId",taskId);m.put("kind",kind);m.put("changeSetId",changeSetId);m.put("reviewAttemptId",reviewAttemptId);m.put("conversationKey",conversationKey);m.put("conversationTitle",conversationTitle);m.put("artifactPath",artifactPath);m.put("artifactSha256",artifactSha256);m.put("artifactSize",artifactSize);m.put("fileName",fileName);m.put("autoSend",autoSend);m.put("autoGenerated",autoGenerated);m.put("status",status);m.put("claimedTabId",claimedTabId);m.put("message",message);m.put("createdAt",createdAt);m.put("updatedAt",updatedAt);m.put("leaseUntil",leaseUntil);m.put("payloadTicket",payloadTicket);m.put("ticketExpiresAt",ticketExpiresAt);return m;}
        static Task from(Map<String,Object> m){Task t=new Task();t.taskId=Core.str(m.get("taskId"));t.kind=Core.str(m.get("kind"));t.changeSetId=Core.str(m.get("changeSetId"));t.reviewAttemptId=Core.str(m.get("reviewAttemptId"));t.conversationKey=Core.str(m.get("conversationKey"));t.conversationTitle=Core.str(m.get("conversationTitle"));t.artifactPath=Core.str(m.get("artifactPath"));t.artifactSha256=Core.str(m.get("artifactSha256"));Object sz=m.get("artifactSize");t.artifactSize=sz instanceof Number n?n.longValue():-1;t.fileName=Core.str(m.get("fileName"));t.autoSend=Boolean.TRUE.equals(m.get("autoSend"));t.autoGenerated=Boolean.TRUE.equals(m.get("autoGenerated"));t.status=Core.str(m.get("status"));Object tab=m.get("claimedTabId");t.claimedTabId=tab instanceof Number n?n.intValue():null;t.message=Core.str(m.get("message"));t.createdAt=Core.str(m.get("createdAt"));t.updatedAt=Core.str(m.get("updatedAt"));t.leaseUntil=Core.str(m.get("leaseUntil"));t.payloadTicket=Core.str(m.get("payloadTicket"));t.ticketExpiresAt=Core.str(m.get("ticketExpiresAt"));return t;}
    }
}

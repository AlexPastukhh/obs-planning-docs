...
final class ChatBridgeService {
    ...
    private static final Set<String> ACTIONABLE=Set.of("Pending","Claimed","Preparing","SendArmed","SendClicked");
    private final StateStore state;
    private final SecureRandom random=new SecureRandom();
    private final LinkedHashMap<String,Core.ChatConversation> conversations=new LinkedHashMap<>();
    private final ScheduledThreadPoolExecutor snapshotDeadlineExecutor;
    private final LinkedHashMap<String,ScheduledFuture<?>> snapshotDeadlineFutures=new LinkedHashMap<>();
    private final long snapshotConfirmSeconds;
    ...
    ChatBridgeService(StateStore state,long snapshotConfirmSeconds){
        ...
        if(snapshotConfirmSeconds<=0)throw new IllegalArgumentException("Snapshot confirmation deadline must be positive.");
        this.snapshotConfirmSeconds=snapshotConfirmSeconds;
    ...
    synchronized void setEventSink(Consumer<ChatEvent> sink){
        ...
        if(sink==null){deferredEvents.clear();return;}
        while(!deferredEvents.isEmpty()){ChatEvent event=deferredEvents.removeFirst();try{sink.accept(event);}catch(Throwable ignored){}}
    }
    ...
    synchronized Core.ChatTaskInfo enqueueReview(Core.ChangeSet cs,Core.ReviewDiff review,Core.ChatBinding b,boolean manual){
        ...
        Task equivalent=equivalentActionableTask("reviewDiff",cs.changeSetId,review.attemptId(),b.conversationKey(),sha,true);if(equivalent!=null)return info(equivalent);
        if(!manual){
    ...
    synchronized Core.ChatTaskInfo enqueueSnapshot(Path zip,String conversationKey){return enqueueSnapshot(zip,conversationKey,false);}
    synchronized Core.ChatTaskInfo enqueueSnapshot(Path zip,String conversationKey,boolean autoSend){
        expireClaims();Core.ChatConversation c=conversations.get(conversationKey);if(c==null)throw fail("Selected ChatGPT conversation is not currently open.");
        Path p=zip==null?null:zip.toAbsolutePath().normalize();verifySnapshotZip(p);long size=size(p);String sha=Core.sha256(p);
        Task equivalent=equivalentActionableTask("snapshot",null,null,c.conversationKey(),sha,autoSend);if(equivalent!=null)return info(equivalent);
        Task t=new Task();t.taskId=UUID.randomUUID().toString();t.kind="snapshot";t.conversationKey=c.conversationKey();t.conversationTitle=c.title();t.artifactPath=p.toString();t.artifactSha256=sha;t.artifactSize=size;t.fileName=p.getFileName().toString();t.autoSend=autoSend;t.autoGenerated=false;t.sendRetryIntervalSeconds=state.getSettings().reviewDiffSendRetrySeconds();t.status="Pending";t.createdAt=t.updatedAt=Instant.now().toString();save(t);scheduleSnapshotDeadline(t);return info(t);
    }
    ...
    synchronized Map<String,Object> claim(String conversationKey,int tabId){
        ...
        for(Task t:listTasks())if(Objects.equals(t.conversationKey,conversationKey)&&Set.of("Claimed","Preparing","SendArmed","SendClicked").contains(t.status)){
    ...
    synchronized String release(String taskId,int tabId,String conversationKey,String message){
        ...
        if("SendArmed".equals(t.status)){terminal(t,"UnknownAfterSend",message==null?"Claim was lost after Send authorization; a Send click may already have happened.":message);return t.status;}
    ...
    synchronized void releaseTab(int tabId,String message){
        for(Task t:listTasks())if(Objects.equals(t.claimedTabId,tabId)){if("Claimed".equals(t.status))toPending(t,message);else if("Preparing".equals(t.status))terminal(t,"PreparedUnsent",message);else if("SendArmed".equals(t.status)||"SendClicked".equals(t.status))terminal(t,"UnknownAfterSend",message);}
    }
    ...
    synchronized void stageSendArmed(String taskId,int tabId,String conversationKey){
        expireClaims();Task t=requirePreparing(taskId,tabId,conversationKey);if(!t.autoSend)throw fail("Send is not allowed for this task.");
        cancelSnapshotDeadline(t.taskId);t.status="SendArmed";t.updatedAt=Instant.now().toString();t.leaseUntil=Instant.now().plusSeconds(SEND_UNKNOWN_SECONDS).toString();save(t);
    }

    synchronized void stageSendNotClicked(String taskId,int tabId,String conversationKey){
        Task t=requireSendArmed(taskId,tabId,conversationKey);
        if("snapshot".equals(t.kind)&&snapshotDeadlineExpired(t)){terminal(t,"PreparedUnsent","Snapshot Send attempt was definitively not clicked, and the task confirmation window expired while the guarded attempt was in progress; attachment may remain in the composer; no further automation.");return;}
        t.status="Preparing";t.updatedAt=Instant.now().toString();t.leaseUntil=Instant.now().plusSeconds(CLAIM_SECONDS).toString();save(t);if("snapshot".equals(t.kind))scheduleSnapshotDeadline(t);
    }

    synchronized void stageSendClicked(String taskId,int tabId,String conversationKey){
        expireClaims();Task t=requireSendArmed(taskId,tabId,conversationKey);t.status="SendClicked";t.updatedAt=Instant.now().toString();t.leaseUntil=Instant.now().plusSeconds(SEND_UNKNOWN_SECONDS).toString();save(t);
    }
    ...
    synchronized void result(String taskId,int tabId,String conversationKey,String status,String message){
        ...
        if(!Set.of("snapshot","reviewDiff").contains(t.kind))throw fail("Unsupported ChatGPT task kind.");
        boolean beforePrepare="Claimed".equals(t.status)&&Set.of("FailedBeforeSend","PreparedUnsent").contains(status);
        boolean prepared=t.autoSend
                ?(("Preparing".equals(t.status)&&Set.of("PreparedUnsent","UnknownAfterSend").contains(status))||("SendArmed".equals(t.status)&&"UnknownAfterSend".equals(status))||("SendClicked".equals(t.status)&&Set.of("Sent","UnknownAfterSend").contains(status)))
                :"Preparing".equals(t.status)&&Set.of("Attached","PreparedUnsent").contains(status);
        if(!(beforePrepare||prepared))throw fail("Invalid "+t.kind+" task transition "+t.status+" → "+status+".");
    ...
    synchronized List<Core.ExternalInteraction> externalInteractions(){
        ...
        for(Task t:listTasks())if(!TERMINAL.contains(t.status)||("UnknownAfterSend".equals(t.status)&&t.dismissedAt==null))out.add(interaction(t));
    ...
    synchronized Core.ExternalInteraction cancelExternalInteraction(String taskId){
        ...
        if("SendArmed".equals(t.status)||"SendClicked".equals(t.status))throw fail("Send may already have happened; cancellation cannot rewrite this interaction as Cancelled.");
        throw fail("Interaction is already terminal in state "+t.status+".");
    }

    synchronized Core.ExternalInteraction dismissExternalInteraction(String taskId){
        expireClaims();Task t=requireTask(taskId);
        if(!"UnknownAfterSend".equals(t.status))throw fail("Only terminal UnknownAfterSend interactions require acknowledgement/dismissal.");
        if(t.dismissedAt==null){t.dismissedAt=Instant.now().toString();t.updatedAt=t.dismissedAt;save(t);}
        return interaction(t);
    }

    private Core.ExternalInteraction interaction(Task t){String kind="reviewDiff".equals(t.kind)?"Deliver Current Change":t.autoSend?"Send Repository Snapshot":"Attach Repository Snapshot";String source=t.fileName==null?t.artifactPath:t.fileName;String title=t.conversationTitle==null||t.conversationTitle.isBlank()?"ChatGPT conversation":t.conversationTitle;String destination=title+" ["+t.conversationKey+"]";String semantic=switch(t.status){case "Pending","Claimed"->"Pending";case "Preparing"->"Preparing";case "SendArmed","SendClicked"->"Sending";default->t.status;};boolean cancellable=Set.of("Pending","Claimed","Preparing").contains(t.status);return new Core.ExternalInteraction(t.taskId,kind,t.changeSetId,source,destination,semantic,t.message,t.updatedAt,cancellable);}
    ...
    private Task requireSendArmed(String taskId,int tabId,String conversationKey){Task t=requireTask(taskId);if(!"SendArmed".equals(t.status)||!Objects.equals(t.claimedTabId,tabId))throw fail("ChatGPT task has no active Send authorization in this tab.");if(!Objects.equals(t.conversationKey,conversationKey)||!tabBelongs(conversationKey,tabId))throw fail("Send-authorized ChatGPT tab is no longer on the task conversation.");return t;}
    ...
    private Task equivalentActionableTask(String kind,String changeSetId,String reviewAttemptId,String conversationKey,String artifactSha256,boolean autoSend){for(Task t:listTasks())if(ACTIONABLE.contains(t.status)&&Objects.equals(t.kind,kind)&&Objects.equals(t.changeSetId,changeSetId)&&Objects.equals(t.reviewAttemptId,reviewAttemptId)&&Objects.equals(t.conversationKey,conversationKey)&&Objects.equals(t.artifactSha256,artifactSha256)&&t.autoSend==autoSend)return t;return null;}
    ...
    private void requireNoUnsafeInFlight(String changeSetId,String message){for(Task t:listTasks())if("reviewDiff".equals(t.kind)&&Objects.equals(t.changeSetId,changeSetId)&&Set.of("Preparing","SendArmed","SendClicked").contains(t.status))throw fail(message);}
    private void cancel(Task t,String message){terminal(t,"Cancelled",message);}
    private void terminal(Task t,String status,String message){cancelSnapshotDeadline(t.taskId);t.status=status;t.message=message;t.payloadTicket=null;t.ticketExpiresAt=null;t.leaseUntil=null;t.updatedAt=Instant.now().toString();save(t);emit(t);}
    ...
    private void restoreSnapshotDeadlines(){
        for(Task t:listTasks())if(activeSnapshot(t)){
            if(snapshotDeadlineExpired(t))expireSnapshotDeadline(t);else scheduleSnapshotDeadline(t);
        }
    }
    private boolean activeSnapshot(Task t){return "snapshot".equals(t.kind)&&Set.of("Pending","Claimed","Preparing").contains(t.status);}
    private Instant snapshotDeadline(Task t){try{return Instant.parse(t.createdAt).plusSeconds(snapshotConfirmSeconds);}catch(Exception e){return Instant.EPOCH;}}
    private boolean snapshotDeadlineExpired(Task t){return !snapshotDeadline(t).isAfter(Instant.now());}
    private void scheduleSnapshotDeadline(Task t){
        if(!activeSnapshot(t))return;
        cancelSnapshotDeadline(t.taskId);
        long delay=Math.max(0,Duration.between(Instant.now(),snapshotDeadline(t)).toMillis());
        snapshotDeadlineFutures.put(t.taskId,snapshotDeadlineExecutor.schedule(()->snapshotDeadlineFired(t.taskId),delay,TimeUnit.MILLISECONDS));
    }
    private void cancelSnapshotDeadline(String taskId){ScheduledFuture<?> f=snapshotDeadlineFutures.remove(taskId);if(f!=null)f.cancel(false);}
    private synchronized void snapshotDeadlineFired(String taskId){
        snapshotDeadlineFutures.remove(taskId);Task t=load(taskId);
        if(t==null||!activeSnapshot(t))return;
        if(!snapshotDeadlineExpired(t)){scheduleSnapshotDeadline(t);return;}
        expireSnapshotDeadline(t);
    }
    private void expireSnapshotDeadline(Task t){
        if("Preparing".equals(t.status))terminal(t,"PreparedUnsent","Snapshot attachment was not confirmed within the task confirmation window; attachment may remain in the composer; no further automation or Send.");
        else terminal(t,"Cancelled","Snapshot attachment was not confirmed within the task confirmation window before external preparation was confirmed; no further automation.");
    }

    private void expireClaims(){
        for(Task t:listTasks()){
            if(activeSnapshot(t)&&snapshotDeadlineExpired(t)){expireSnapshotDeadline(t);continue;}
            if("Claimed".equals(t.status)&&expired(t.leaseUntil))toPending(t,"Claim lease expired before composer preparation.");
            else if("Preparing".equals(t.status)&&expired(t.leaseUntil))terminal(t,"PreparedUnsent","Composer preparation lease expired before Send authorization; inspect the unsent draft before retrying.");
            else if("SendArmed".equals(t.status)&&expired(t.leaseUntil))terminal(t,"UnknownAfterSend","A guarded Send attempt was authorized but its click outcome could not be confirmed before the safety timeout.");
            else if("SendClicked".equals(t.status)&&expired(t.leaseUntil))terminal(t,"UnknownAfterSend","Automatic Send attempts could not be confirmed before the safety timeout.");
        }
    }
    private void expireClaimsAgainstInventory(){for(Task t:listTasks())if(t.claimedTabId!=null&&!tabBelongs(t.conversationKey,t.claimedTabId)){if("Claimed".equals(t.status))toPending(t,"Claimed tab left the task conversation before composer preparation.");else if("Preparing".equals(t.status))terminal(t,"PreparedUnsent","Claimed tab left the task conversation after composer preparation began; inspect the unsent draft before retrying.");else if("SendArmed".equals(t.status))terminal(t,"UnknownAfterSend","Claimed tab left the task conversation after Send authorization; a Send click may already have happened.");else if("SendClicked".equals(t.status))terminal(t,"UnknownAfterSend","Claimed tab left the task conversation after SendClicked.");}}
    ...
    private void emit(Task t){ChatEvent event=new ChatEvent(t.taskId,t.kind,t.changeSetId,t.reviewAttemptId,t.status,t.message);if(!eventSinkConfigured){deferredEvents.addLast(event);return;}if(eventSink==null)return;try{eventSink.accept(event);}catch(Throwable ignored){}}

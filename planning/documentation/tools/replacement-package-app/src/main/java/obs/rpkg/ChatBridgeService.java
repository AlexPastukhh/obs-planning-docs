...
final class ChatBridgeService {
    ...
    synchronized void result(String taskId,int tabId,String conversationKey,String status,String message){
        ...
        if("snapshot".equals(t.kind)){
            ...
            boolean ok=("Claimed".equals(t.status)&&Set.of("FailedBeforeSend","PreparedUnsent").contains(status))||("Preparing".equals(t.status)&&"PreparedUnsent".equals(status))||("SendClicked".equals(t.status)&&Set.of("Sent","UnknownAfterSend").contains(status));if(!ok)throw fail("Invalid ReviewDiff task transition "+t.status+" → "+status+".");

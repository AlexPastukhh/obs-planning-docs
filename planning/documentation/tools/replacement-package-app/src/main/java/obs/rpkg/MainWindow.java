...
final class MainWindow extends JFrame {
    ...
    private final JTextField repositoryIdentity=new JTextField(),archive=new JTextField(),changeSetId=new JTextField(),status=new JTextField(),currentSha=new JTextField(),reviewedSha=new JTextField(),commitMessage=new JTextField("Update reviewed ChangeSet");
    private final JTextArea action=new JTextArea(7,60),log=new JTextArea(12,60);

    MainWindow(Core core){super("OBS Replacement Package App — Java 21");this.core=core;setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);setMinimumSize(new Dimension(980,760));for(JTextField f:new JTextField[]{repositoryIdentity,changeSetId,status,currentSha})f.setEditable(false);log.setEditable(false);build();loadState();pack();setLocationRelativeTo(null);}
    ...
    private void build(){
        ...
        root.add(row("Current Review SHA-256",currentSha));
        root.add(row("",button("Refresh Review",this::refreshReview),button("Copy ReviewDiff",this::copyReviewDiff),button("Open ReviewDiff",this::openReviewDiff),button("Approve Current Review",this::approveCurrentReview)));
        root.add(row("Reviewed SHA-256",reviewedSha));root.add(row("Commit message",commitMessage));
    ...
    private void changeSetChanged(){ChangeSetItem item=(ChangeSetItem)changeSets.getSelectedItem();selectedChangeSet=item==null?null:item.value;if(selectedChangeSet==null){core.selectChangeSet(null);clearChangeSetFields();return;}core.selectChangeSet(selectedChangeSet.changeSetId);changeSetId.setText(selectedChangeSet.changeSetId);status.setText(selectedChangeSet.status);reviewedSha.setText("");try{currentReview=core.currentReview(selectedChangeSet);currentSha.setText(currentReview==null?"":currentReview.sha256());}catch(Core.ObsException e){currentReview=null;currentSha.setText("");append("WARNING Stored current ReviewDiff is unavailable: "+e.getMessage());}}
    ...
    private void clearChangeSetFields(){selectedChangeSet=null;currentReview=null;changeSetId.setText("");status.setText("");currentSha.setText("");reviewedSha.setText("");}
    ...
    private void approveCurrentReview(){if(!requireCurrentReview())return;core.verifiedReviewDiffPath(currentReview);reviewedSha.setText(currentReview.sha256());append("SUCCESS Current Review approved for Finalize: "+currentReview.sha256());}
    private void finalizeChangeSet(){if(selectedChangeSet==null)throw new Core.ObsException(Core.FINALIZE_FAILED,"Select a ChangeSet first.");Core.FinalizeResult r=core.finalizeChangeSet(selectedChangeSet.changeSetId,reviewedSha.getText().trim(),commitMessage.getText(),repoPath());append(r.commitSha()==null?"SUCCESS Finalized with no net changes; no commit/push required.":"SUCCESS Finalized commit "+r.commitSha());reloadChangeSets(null);}

...
final class MainWindow extends JFrame {
    ...
    private void build(){
        ...
        root.add(row("ChangeSet",changeSets,button("Start workspace",this::startChangeSetWorkspace)));
    ...
    private void completeApply(Core.ApplyResult r,Core.RepositoryConfig target){
        String cs=r.changeSet().changeSetId;appendToOutput(cs,r.review()==null?"SUCCESS Apply. Package files are AppliedUncommitted in the isolated ChangeSet worktree; Commit/Review migration is pending.":"SUCCESS Apply. ReviewDiff is current.");if(r.diagnostic()!=null&&!r.diagnostic().isBlank())appendToOutput(cs,"WARNING "+r.diagnostic());if(r.attempt().handoffWarning!=null&&!r.attempt().handoffWarning.isBlank())appendToOutput(cs,"WARNING "+r.attempt().handoffWarning);reloadChangeSets(cs);notifyOperation("Apply succeeded",r.changeSet().changeSetLabel,target.id(),false);
    }
    ...
    private static String displayChangeSetStatus(Core.ChangeSet cs){String lifecycle=displayLifecycle(cs.status);return cs.executionState==null||cs.executionState.isBlank()?lifecycle:lifecycle+" · "+cs.executionState+(cs.publishedTip==null||cs.publishedTip.isBlank()?"":" @ "+shortSha(cs.publishedTip));}

...
final class MainWindow extends JFrame {
    ...
    private void build(){
        ...
        root.add(row("",button("Refresh interactions",this::refreshInteractions),button("Cancel interaction",this::cancelInteraction)));
    ...
    private void sendCurrentReviewToChat(){if(selectedChangeSet==null||requireCurrentReview()==null)return;String repoId=selectedRepository==null?null:selectedRepository.id(),cs=selectedChangeSet.changeSetId;try{Core.ChatTaskInfo t=core.sendCurrentReviewToChat(cs);if("NoChanges".equals(t.status())){appendToOutput(cs,"SUCCESS Current Change is empty; no ChatGPT message was sent.");core.recordOperationOutcome(cs,"SUCCESS",Core.SUCCESS,"No current change to send.");notifyOperation("Current Change · No changes","No ChatGPT message was sent.",repoId,false);}else appendToOutput(cs,"SUCCESS ReviewDiff queued for ChatGPT: "+t.conversationTitle()+" · "+t.taskId().substring(0,8));updateChatDelivery();refreshInteractions();}catch(Throwable e){trackedFailure("Deliver Current Change",e,repoId,cs);}}
    ...
    private void exportRepositorySnapshot(){
        ...
        String repositoryId=selectedRepository.id();append("INFO Exporting Repository Snapshot…");
        runBackground("Repository Snapshot",()->core.exportRepositorySnapshot(repository,selectedMode,commitRef,out),r->{Core.Handoff clip=core.copyPathToClipboard(r.zipPath());append("SUCCESS Repository snapshot created: "+r.zipPath().toAbsolutePath().normalize());if(clip.warning()!=null&&!clip.warning().isBlank())append("WARNING Snapshot created, but path was not copied to clipboard: "+clip.warning());else append("SUCCESS Snapshot path copied to clipboard.");notifyOperation("Repository snapshot created",r.zipPath().getFileName().toString(),repositoryId,false);showSnapshotResult(r,clip);},e->trackedFailure("Export Repository Snapshot",e,repositoryId,null));
    }

...
final class MainWindow extends JFrame {
    ...
    private static String displayChangeSetStatus(Core.ChangeSet cs){String lifecycle=displayLifecycle(cs.status);if(cs.executionState==null||cs.executionState.isBlank())return lifecycle;if(Set.of("CommittedUnpublished","PublicationUncertain").contains(cs.executionState)&&cs.commitSha!=null&&!cs.commitSha.isBlank())return lifecycle+" · "+cs.executionState+" · "+shortSha(cs.publishedTip)+" → "+shortSha(cs.commitSha);return lifecycle+" · "+cs.executionState+(cs.publishedTip==null||cs.publishedTip.isBlank()?"":" @ "+shortSha(cs.publishedTip));}

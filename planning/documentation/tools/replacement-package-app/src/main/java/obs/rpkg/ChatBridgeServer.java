...
final class ChatBridgeServer implements AutoCloseable {
    ...
    private void handle(HttpExchange x)throws IOException{
        try{
            ...
            if(path.equals("/v1/health")){auth(x);sendJson(x,200,Map.of("ok",true,"port",ChatBridgeService.PORT));return;}
            if(path.equals("/v1/inventory")&&"POST".equalsIgnoreCase(x.getRequestMethod())){auth(x);Map<String,Object> body=objectBody(x);List<Map<String,Object>> convs=new ArrayList<>();Object raw=body.get("conversations");if(raw instanceof List<?> l)for(Object item:l)if(item instanceof Map<?,?> m)convs.add(cast(m));service.replaceInventory(convs);sendJson(x,200,Map.of("ok",true));return;}
            if(path.equals("/v1/tasks/claim")&&"POST".equalsIgnoreCase(x.getRequestMethod())){auth(x);Map<String,Object> body=objectBody(x);String key=Core.str(body.get("conversationKey"));int tab=num(body.get("tabId"));Map<String,Object> task=service.claim(key,tab);sendJson(x,200,task.isEmpty()?Map.of("task",null):Map.of("task",task));return;}

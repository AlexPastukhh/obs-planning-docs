...
final class ChatBridgeServer implements AutoCloseable {
    ...
    private void handle(HttpExchange x)throws IOException{
        try{
            ...
            if(path.equals("/v1/chat-context/wait")&&"POST".equalsIgnoreCase(x.getRequestMethod())){auth(x);Map<String,Object> body=objectBody(x);requireBridgeProtocol(body);long after=longNum(body.get("afterRevision"),0),timeout=longNum(body.get("timeoutMs"),20_000);ChatBridgeService.ContextLookupBatch batch=service.waitForContextLookupRequests(after,timeout);Map<String,Object> response=new LinkedHashMap<>();response.put("ok",true);response.put("bridgeProtocolVersion",ChatBridgeService.BRIDGE_PROTOCOL_VERSION);response.put("revision",batch.revision());response.put("contextLookups",batch.lookups());sendJson(x,200,response);return;}
            if(path.equals("/v1/chat-context/result")&&"POST".equalsIgnoreCase(x.getRequestMethod())){auth(x);Map<String,Object> body=objectBody(x);requireBridgeProtocol(body);String token=Core.str(body.get("chatContextToken"));List<Map<String,Object>> captures=new ArrayList<>();Object raw=body.get("captures");if(raw instanceof List<?> l)for(Object item:l)if(item instanceof Map<?,?> m)captures.add(cast(m));String status=service.acceptContextLookupResult(token,captures);sendJson(x,200,Map.of("ok",true,"status",status));return;}
    ...
    private static long longNum(Object o,long fallback){return o instanceof Number n?n.longValue():fallback;}

...
public final class Core {
    ...
    private CommitResult commitAppliedPackageInternal(String id){
        try(StateStore.Lock ignored=state.lock()){
            ...
            String commit=git.run(worktree,COMMIT_FAILED,"rev-parse","HEAD").first();verifyExactPackageCommit(repositoryTarget,cs,worktree,repositoryIdentity,journal,commit);cs.executionState="CommittedUnpublished";cs.commitSha=commit;cs.updatedAt=Instant.now().toString();setOutcome(cs,"SUCCESS",SUCCESS,"Local package commit created; publication is pending.");state.saveChangeSet(cs);return new CommitResult(SUCCESS,commit,cs,false);
        }
    }
    private void assertApplyJournalChangeSet(ApplyJournal journal,ChangeSet cs,String repositoryIdentity,Path worktree){if(!Objects.equals(journal.changeSetId,cs.changeSetId)||!Objects.equals(journal.packageId,cs.lastPackageId)||!same(journal.repositoryIdentity,repositoryIdentity)||!Objects.equals(journal.branch,cs.branch)||!samePath(Path.of(journal.worktree),worktree)||!Objects.equals(journal.baseHead,cs.publishedTip))throw new ObsException(STATE_DIVERGED,"Durable Apply journal does not match the persisted Git-backed ChangeSet execution state.");}
    ...
    private void verifyExactPackageCommit(Path repositoryTarget,ChangeSet cs,Path worktree,String repositoryIdentity,ApplyJournal journal,String commit){WorkspacePosition position=verifyWorkspacePosition(repositoryTarget,cs,worktree,repositoryIdentity);if(!Objects.equals(position.head,commit)||!Objects.equals(position.branchTip,commit))throw new ObsException(STATE_DIVERGED,"Expected package commit is not the exact ChangeSet branch/worktree tip.");String parentLine=git.run(worktree,STATE_DIVERGED,"rev-list","--parents","-n","1",commit).first();String[] parents=parentLine.isBlank()?new String[0]:parentLine.split("\\s+");if(parents.length!=2||!Objects.equals(parents[1],cs.publishedTip))throw new ObsException(STATE_DIVERGED,"Package commit is not a single-parent child of publishedTip "+cs.publishedTip+".");if(!commitHasExactTrailers(worktree,commit,journal.packageId,cs.changeSetId))throw new ObsException(STATE_DIVERGED,"Package commit trailers do not prove the durable package/ChangeSet identity.");Set<String> allowed=journalPathSet(journal);for(String path:gitNulPaths(worktree,"diff","--name-only","-z",cs.publishedTip,commit,"--"))if(!allowed.contains(path))throw new ObsException(STATE_DIVERGED,"Package commit changed a path outside its durable package paths: "+path);GitClient.Result index=git.allow(worktree,STATE_DIVERGED,"diff","--cached","--quiet");if(index.exitCode()!=0)throw new ObsException(STATE_DIVERGED,index.exitCode()==1?"Package commit recovery requires a clean real Git index.":"Cannot verify Git index after package commit.\n--- git details ---\n"+index.failureDetails());if(!allJournalState(worktree,journal,true))throw new ObsException(STATE_DIVERGED,"Package commit worktree no longer matches the durable intended package result.");if(!gitNulPaths(worktree,"diff","--name-only","-z","HEAD","--").isEmpty()||!gitNulPaths(worktree,"ls-files","--others","-z","--").isEmpty())throw new ObsException(STATE_DIVERGED,"Package commit worktree contains changes after the expected commit.");}

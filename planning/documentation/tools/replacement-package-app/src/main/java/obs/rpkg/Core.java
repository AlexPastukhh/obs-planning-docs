...
public final class Core {
    ...
    private ApplyResult applyInternal(PackageData pkg,Path repositoryRoot,ObsAction action,ReviewChatBindingDecision reviewChatDecision,ReviewChatBindingPlan reviewChatPlan){
        String attemptId=UUID.randomUUID().toString(),now=Instant.now().toString();Path repo=null;ChangeSet cs=null;ReviewDiff review=null;ApplicationAttempt success=null;String labelDiagnostic="";StateStore.Lock stateLock=state.lock();try{
            ...
            List<PackageStateApplier.Operation> fileOperations=new ArrayList<>();for(Operation op:pkg.manifest.operations){Path target=inside(repo,op.path);PackageStateApplier.Action sharedAction=switch(op.action){case"add"->PackageStateApplier.Action.ADD;case"replace"->PackageStateApplier.Action.REPLACE;case"delete"->PackageStateApplier.Action.DELETE;default->throw new ObsException(PACKAGE_INVALID,"Unsupported action '"+op.action+"' for "+op.path);};fileOperations.add(new PackageStateApplier.Operation(op.path,target,sharedAction,pkg.base.get(op.path),pkg.replacement.get(op.path)));}
            Path verifiedRepo=repo;PackageStateApplier.PreparedChange preparedFiles;try{preparedFiles=PackageStateApplier.prepare(fileOperations,(path,expected,actual)->requireExpectedSource(verifiedRepo,path,expected,actual));}catch(Throwable t){throw mapPackageStateFailure(t);}

...
public final class Core {
    ...
    private ApplyResult applyGitBackedPackageUnlocked(PackageData pkg,ObsAction action,ReviewChatBindingDecision reviewChatDecision,ReviewChatBindingPlan reviewChatPlan,String attemptId,String now,RepositoryConfig allowed,String repoIdentity,Path worktree,ChangeSet cs,byte[] priorState,String labelDiagnostic){
        ...
        if(!Set.of("Ready","AppliedUncommitted").contains(cs.executionState))throw new ObsException(STATE_DIVERGED,"Git-backed package Apply is not available from execution state: "+cs.executionState);
        ...
        if("AppliedUncommitted".equals(cs.executionState)){
            ...
            assertOnlyJournalPathsDirty(worktree,journal);if(!allJournalState(worktree,journal,true))throw new ObsException(APPLY_ROLLBACK_UNVERIFIED,"AppliedUncommitted worktree no longer matches the durable intended package result.");

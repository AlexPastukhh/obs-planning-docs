...
public final class Core {
    ...
    public record Settings(String repositoryRoot,String reviewDiffHandling) {}
    ...
    private final GitClient git=new GitClient(); private final StateStore state; private Runnable afterMutationHook=()->{};
    public Core(){this(new StateStore());}
    Core(StateStore state){this.state=state;}
    void setAfterMutationHookForTests(Runnable hook){afterMutationHook=hook==null?()->{}:hook;}

    public Settings getSettings(){return state.getSettings();}
    public Settings setSettings(String repositoryRoot,String handling){if(!List.of("Clipboard","RepoDiffFile","Both").contains(handling))throw new ObsException(PACKAGE_INVALID,"Unknown ReviewDiff handling: "+handling);Settings s=new Settings(repositoryRoot==null?"":repositoryRoot,handling);state.saveSettings(s);return s;}
    ...
    private ApplyResult applyInternal(PackageData pkg,Path repositoryRoot,ObsAction action){
        String attemptId=UUID.randomUUID().toString(),now=Instant.now().toString();Path repo=null;ChangeSet cs=null;ReviewDiff review=null;ApplicationAttempt success=null;StateStore.Lock stateLock=state.lock();try{
            repo=repoRoot(repositoryRoot);String repoIdentity=repositoryIdentity(repo);if(!same(repoIdentity,pkg.manifest.repositoryIdentity))throw new ObsException(REPOSITORY_MISMATCH,"Configured origin is "+repoIdentity+"; package targets "+pkg.manifest.repositoryIdentity+".");
    ...
    public Handoff publishReviewDiff(ChangeSet cs,ReviewDiff review){Settings s=state.getSettings();String handling=s.reviewDiffHandling;String service=null;List<String>w=new ArrayList<>();if(handling.equals("Clipboard")||handling.equals("Both")){try{String text=Files.readString(review.diffPath,StandardCharsets.UTF_8);Toolkit.getDefaultToolkit().getSystemClipboard().setContents(new StringSelection(text),null);}catch(Throwable e){w.add("Clipboard handoff failed.");}}if(handling.equals("RepoDiffFile")||handling.equals("Both")){try{Path rel=Path.of("_ai-review-diffs",cs.changeSetId,review.attemptId+".diff");Path dst=inside(Path.of(cs.repositoryRoot),rel.toString().replace('\\','/'));Files.createDirectories(dst.getParent());Files.copy(review.diffPath,dst,StandardCopyOption.REPLACE_EXISTING);service=rel.toString().replace('\\','/');}catch(Throwable e){w.add("Repo diff-file handoff failed: "+e.getMessage());}}return new Handoff(service,String.join(" ",w));}
    ...
    public FinalizeResult finalizeChangeSet(String id,String reviewedSha,String message,Path repositoryRoot){
        try(StateStore.Lock ignored=state.lock()){
            ...
            Path repo=repoRoot(repositoryRoot==null?Path.of(cs.repositoryRoot):repositoryRoot);
    ...
    public FinalizeResult retryPush(String id,Path repositoryRoot){try(StateStore.Lock ignored=state.lock()){ChangeSet cs=state.getChangeSet(id);if(cs==null||!"CommittedPendingPush".equals(cs.status))throw new ObsException(FINALIZE_FAILED,"ChangeSet is not CommittedPendingPush.");Path repo=repoRoot(repositoryRoot==null?Path.of(cs.repositoryRoot):repositoryRoot);if(!samePath(repo,Path.of(cs.repositoryRoot)))throw new ObsException(REPOSITORY_MISMATCH,"Retry Push repository differs from ChangeSet repository.");String rid=repositoryIdentity(repo);if(!same(rid,cs.repositoryIdentity))throw new ObsException(REPOSITORY_MISMATCH,"Retry Push origin is "+rid+"; ChangeSet targets "+cs.repositoryIdentity+".");String head=git.run(repo,STATE_DIVERGED,"rev-parse","HEAD").first();if(!head.equals(cs.commitSha))throw new ObsException(STATE_DIVERGED,"HEAD is not the recorded pending-push commit.");GitClient.Result push=git.allow(repo,FINALIZE_FAILED,"push","origin",cs.branch);if(push.exitCode()!=0)throw new ObsException(FINALIZE_FAILED,"Retry Push failed; existing commit remains pending.");cs.status="Finalized";cs.updatedAt=Instant.now().toString();state.saveChangeSet(cs);return new FinalizeResult(SUCCESS,cs.commitSha,cs.branch,cs);}}

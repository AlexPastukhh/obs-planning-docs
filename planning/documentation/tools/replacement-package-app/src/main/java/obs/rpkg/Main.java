...
public final class Main {
    public static void main(String[] args) {
        ...
        try {
            ...
            switch (args[0]) {
                ...
                case "review" -> {
                    Core.ChangeSet cs = core.getChangeSet(req(a,"changeset"));
                    if (cs == null) throw new Core.ObsException(Core.STATE_DIVERGED,"Unknown ChangeSet");
                    Core.ReviewDiff r = core.newReviewDiff(cs);
                ...
                case "settings" -> {
                    String repo=a.getOrDefault("repo","");String handling=a.getOrDefault("review-diff","Clipboard");Core.Settings s=core.setSettings(repo,handling);System.out.println("SUCCESS repositoryRoot="+s.repositoryRoot()+" reviewDiffHandling="+s.reviewDiffHandling());
                }
                default -> usage();
            }
    ...
    private static void usage(){System.out.println("Usage: java -jar replacement-package-app.jar [ui|apply|review|finalize|retry-push|settings] ...");}
}

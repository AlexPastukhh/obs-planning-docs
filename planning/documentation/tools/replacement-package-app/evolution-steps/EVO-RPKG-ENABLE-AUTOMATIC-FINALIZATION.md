...
- [Introduce Work Finalization](EVO-RPKG-INTRODUCE-WORK-FINALIZATION.md) realized/materialized

## Realization Prerequisite
- [`Standardize Typed Operation Results`](EVO-RPKG-STANDARDIZE-OPERATION-RESULTS.md)

## Step Purpose
Support both explicit immediate Apply→Finalize composition and deferred independent Finalize without merging the two Features.

## Owner Impacts
### Scenario Impact — CHANGED
Adds SR `Support Immediate And Deferred Finalization` and two selected branches.

### Feature Impact — Apply Replacement Package — CHANGED
Adds `FinalizeMode` and `Automatic Finalize Only When Requested`. Finalize Feature is inherited unchanged from predecessor and remains independently callable.

### Slice Impact — CHANGED
Adds one-way orchestration from eligible Apply result to separate Finalize Feature when explicitly requested.

## Complete Target Feature Body — Apply Replacement Package
...
## Inherited Target Feature — Finalize Repository Work
Use the realized/materialized Finalize Feature from `EVO-RPKG-INTRODUCE-WORK-FINALIZATION`; this Step does not redefine its FBS/BR semantics.
...
|---|---|---|---|---|
| <a id="sps-rpkg-establish-realization-request-01"></a>**Establish Realization Request** `SPS-RPKG-ESTABLISH-REALIZATION-REQUEST-01` | Actor/automation supplies exact realization request. | App entry | WorkId + Package Identity + ApplyExtent + PackageWaitPolicy + FinalizeMode | exact captured request |
...
- Q [BLOCKING]: complete the orchestration Slice body/IR contract before REPLACE materialization.

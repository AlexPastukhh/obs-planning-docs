...
Status: active current reusable command-owner workflow
Scope: current ChatGPT-side producer contract for the `давай архив` / `build replacement archive` command. This workflow defines the currently implemented/documented package creation + current handoff boundary. Planned Builder Start Work/replay/review behavior is owned separately by `scenarios/SCN-BLDR-BUILD-AND-REVIEW-REPLACEMENT-PACKAGE.md`.

This file is colocated with Replacement Package App planning because Builder and App form one connected workflow, but current versus planned authority remains explicit.

Canonical command definition:

`planning/commands/build-replacement-archive.command.md`

Canonical connected planning entry:

`planning/documentation/tools/replacement-package-app/README.md`

Planned Builder Scenario:

`planning/documentation/tools/replacement-package-app/scenarios/SCN-BLDR-BUILD-AND-REVIEW-REPLACEMENT-PACKAGE.md`

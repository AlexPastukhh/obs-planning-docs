...
Current ReviewDiff delivery prepares every non-empty current change as one exact `.diff` attachment through the same low-level attachment primitive used by snapshot handoff. Before any payload/composer mutation, both Java and extension require bridge protocol `2`, and the complete claimed task contract is validated; a stale/incompatible Java bridge or invalid send interval is `FailedBeforeSend` with restart/update guidance, never false `UnknownAfterSend`. `Preparing` is reached only after the task-specific ReviewDiff attachment is visible/upload-ready. The composer must remain free of unrelated text through the MAIN-world pre-click guard. An actual click establishes the possible-Send boundary. After the prepared attachment leaves the composer, a new post-baseline user turn is sufficient to confirm `Sent`; when ChatGPT exposes a turn-local `.diff` file/attachment surface the adapter still records that stronger evidence, but unstable attachment-card metadata is no longer allowed to create false uncertainty after an otherwise observed Send. Attachment loss before any click is `PreparedUnsent`; loss after a possible click with no post-baseline user turn is `UnknownAfterSend`. Guarded attempts use the per-task interval captured from the persisted `Review send retry` application setting (default 6 seconds, valid 1–60). Snapshot remains attach-only. Equivalent still-actionable External Interactions are represented once per exact payload/source + destination (and current ReviewDiff identity where applicable), so repeated queue requests do not create indistinguishable Pending rows; after a terminal outcome, a later retry receives a new interaction identity. Ordinary terminal External Interactions leave the user list after Output/notification. Unacknowledged `UnknownAfterSend` remains visible for attention, but `Dismiss interaction` can persist an acknowledgement and remove that terminal row without changing its truth or deleting its task record. Ownership/adoptability failures identify exact target/path/applying work and explicitly distinguish unowned local changes from another ChangeSet owner.
...
Repository snapshot export remains read-only and independent of ChangeSet lifecycle. Current Local/Committed format is in [`REPOSITORY-SNAPSHOT.md`](REPOSITORY-SNAPSHOT.md). The Swing Snapshot dialog has two explicit paths: `Export only`, or `Export + Attach` after selecting one currently open ordinary ChatGPT conversation in that same dialog. The combined path freezes the selected `conversationKey` before background export; after a successful ZIP exists it queues that exact artifact only to that exact conversation. There is no late post-export destination dialog, no substitution from a later/current chat selection and no mutation of Review-chat binding. The host does not add a special fresh-inventory handshake for tab closure: if the current inventory already knows the destination is unavailable, enqueue is rejected immediately; if stale inventory still permits a task, that exact frozen-destination Snapshot interaction has a fixed 10-minute confirmation window and terminates as `Cancelled` before confirmed external preparation or `PreparedUnsent` after preparation began rather than remaining actionable indefinitely.

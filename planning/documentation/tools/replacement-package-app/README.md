...
`OBS-ACTION/1` may carry optional `chatContextToken: <UUID>` from an explicit Helper `Bind + ...` invocation. Token presence is authoritative for that action: Execute starts an asynchronous bridge lookup while repository Apply proceeds, the extension asks all live ChatGPT agents, and only the tab whose `sessionStorage` contains that captured token returns its click-time `conversationKey`/title/timestamp. If resolution is ready at the successful Apply ReviewDiff cutoff, the normal persisted SL-RPKG-06 binding is established/confirmed and that ReviewDiff is queued. If Apply wins the race, Apply remains successful but that ReviewDiff is not auto-sent; Output/Windows notification says binding was pending. A later resolution may create the binding for future deliveries and emits a separate binding-success notification, without retroactively sending the skipped ReviewDiff. A token resolving to a different existing binding never silently rebinds.

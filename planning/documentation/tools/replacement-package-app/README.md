...
Current ReviewDiff delivery prepares every non-empty current change as one exact `.diff` attachment through the same low-level attachment primitive used by snapshot handoff. Before any payload/composer mutation, both Java and extension require bridge protocol `2`, and the complete claimed task contract is validated; a stale/incompatible Java bridge or invalid send interval is `FailedBeforeSend` with restart/update guidance, never false `UnknownAfterSend`. `Preparing` is reached only after the task-specific ReviewDiff attachment is visible/upload-ready. The composer must remain free of unrelated text through the MAIN-world pre-click guard. An actual click establishes the possible-Send boundary; after the prepared attachment leaves the composer, `Sent` requires a post-baseline outgoing user turn whose complete turn container contains a file/attachment-like DOM surface exposing `.diff`, not ordinary message text. The adapter deliberately expands from the message-author node to the containing conversation turn because live task `44181531` was visibly delivered but the narrower proof returned `UnknownAfterSend`. Its fallback is turn-bounded: it may climb only while the candidate ancestor contains exactly that one authored message node, and stops before crossing into any neighboring authored turn. Full untruncated task-specific filename proof after Send remains pending live DOM inspection. Attachment loss before any click is `PreparedUnsent`; loss after a possible click without that `.diff` attachment-surface confirmation is `UnknownAfterSend`. Guarded attempts use the per-task interval captured from the persisted `Review send retry` application setting (default 6 seconds, valid 1–60). Snapshot remains attach-only in this correction. Equivalent still-actionable External Interactions are represented once per exact payload/source + destination (and current ReviewDiff identity where applicable), so repeated queue requests do not create indistinguishable Pending rows; after a terminal outcome, a later retry receives a new interaction identity. Ordinary terminal External Interactions leave the user list after Output/notification; uncertainty remains attention-requiring state. Ownership/adoptability failures identify exact target/path/applying work and explicitly distinguish unowned local changes from another ChangeSet owner.

...
`Start workspace`, `Apply`, `Commit applied`, `Publish`, Current Change refresh/copy/open, Finalize, Retry Push, Reopen, repository selection/location change, Snapshot export, binding, Attach/Send, External Interaction management and notifications are actions/branches/Slices. They are not separate Scenarios merely because they have separate controls.

## Current implementation boundary

Current source/tests realize legacy `SL-RPKG-01..09`, `SL-RPKG-11 Start ChangeSet Workspace`, and three modular Git-backed execution actions inside the expanded `SL-RPKG-01`: **Apply package files**, **Commit applied package**, and **Publish applied commit**. A Git-backed ChangeSet now progresses `Ready(C0) → AppliedUncommitted(P1) → CommittedUnpublished(P1,C1) → Ready(C1)` in its isolated worktree/branch. Publish verifies the exact remote branch, advances `publishedTip` only after remote proof, uses `PublicationUncertain` when a side-effecting push cannot be reconciled, retains the latest Apply journal as the latest published package boundary, and still acquires no Path Ownership or Repository Target main-workspace mutation. Git-derived Current Change, PR/ReviewDecision and integration Finalize are not migrated yet, so these ChangeSets remain intentionally fenced from legacy Review/Finalize while the existing legacy `SL-RPKG-01..09` flow otherwise remains available for legacy ChangeSets.

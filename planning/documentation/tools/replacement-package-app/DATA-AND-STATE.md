...
`chat-context-lookups/<chatContextToken>.json` stores the consumer-side asynchronous resolution request for one invocation-scoped token: owning `changeSetId` + `packageId`, `Pending` / `WaitingAfterCutoff` / `Bound` / `Rebound` / `Conflict` state, captured conversation/title/timestamp when found, the ReviewDiff cutoff marker and whether the skipped-delivery notice was emitted. Reusing one token for a different package/ChangeSet is rejected. Repository Apply failure is not a token terminal state: unresolved lookup remains pending because the explicit Helper Bind invocation is independent binding authority, and a later unique result still writes the captured conversation as the persisted binding/rebinding. This Java record is separate from the browser capture: Helper keeps the capture itself in that tab's `sessionStorage` and does not delete it after one lookup in this revision. A binding resolved after the Apply delivery cutoff is valid for subsequent deliveries but never retroactively queues the ReviewDiff whose cutoff already passed.
...
The user-facing interaction projection shows active/actionable interactions plus unacknowledged `UnknownAfterSend` (or equivalent attention-requiring uncertainty); it is not terminal history. Ordinary terminal `Cancelled`, `Sent`, `Attached`, `NoChanges`, `FailedBeforeSend` and `PreparedUnsent` interactions leave the projection after their result is surfaced. `UnknownAfterSend` remains immutable in the task ledger but may receive a persisted `dismissedAt` acknowledgement; dismissed uncertainty is omitted from the working projection across restart without changing delivery truth. A retry is a new External Interaction identity; a terminal/cancelled/dismissed interaction is not resumed.

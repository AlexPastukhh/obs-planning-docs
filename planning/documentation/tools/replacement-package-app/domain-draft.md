...
| interaction exact attachment confirmed prepared, failure before possible Send | delivery | PreparedUnsent | yes terminal result | retain prepared-content truth; no blind retry |
| interaction exact attachment remains prepared during Sending | guarded Send attempt | remains same nonterminal interaction | no new identity | another configured attempt is allowed only while source/destination/attachment evidence is unchanged |
| prepared attachment disappears without confirmed outgoing turn | delivery | UnknownAfterSend | yes uncertain result | stop automation; no blind resend |
...
- `INV-RPKG-14` — `Preparing` for current-change delivery means the exact intended ReviewDiff attachment is actually present and upload-ready; a failed preparation attempt before that evidence is not `PreparedUnsent`.
...
- `INV-RPKG-16` — repeated Send-control attempts do not create new External Interaction identity while the same exact ReviewDiff attachment remains prepared in the same destination; attachment disappearance/uncertainty ends that permission and forbids blind resend.

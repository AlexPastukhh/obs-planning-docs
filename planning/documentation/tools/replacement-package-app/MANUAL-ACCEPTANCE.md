...
Register the intended repository, choose WorkId + explicit target branch and Start workspace. First make local target branch stale behind `origin/<targetBranch>` and verify `baseCommit` pins the fresh verified origin tip, not local HEAD. After endpoint verification, add a Git `url.*.insteadOf` rewrite to a different bare remote and verify Start still pins the intended source through isolated transport. Verify one deterministic `changeset/<WorkId>` worktree is created and `GitWorkspace` is persisted with exact Repository Target/worktree/baseCommit. Simulate workspace persistence failure after Git effects and verify retry reconciles the journal-owned workspace rather than creating another branch/worktree; a conflicting leftover journal beside persisted workspace must fail closed.
...
Verify remote absent/previous exact tip permits push with lease; exact intended remote tip is already success; unexpected tip blocks before push. Mutate `origin` immediately after fetch-endpoint verification and prove observation still uses the captured endpoint; mutate `pushurl` immediately after push-endpoint verification and prove the push still reaches only the captured destination. Then add `url.*.insteadOf` after fetch-endpoint verification and `url.*.pushInsteadOf` after push-endpoint verification, each pointing at a foreign bare remote; observation/push must remain on the intended repository and the foreign remote must remain unchanged. Configure a foreign `remote.origin.pushurl` before verification and verify Publish returns repository mismatch before any push. Persist an earlier safe observation, move the remote, and verify a later Publish refreshes observation instead of reusing stale authorization. Force unavailable confirmation after a possible push and verify durable `NotConfirmed`; retry must observe before any further push. Force failure to persist pre-push `NotConfirmed` and prove no push occurs. Force failure to persist final `ConfirmedTip` and prove retry confirms existing remote state without repush.

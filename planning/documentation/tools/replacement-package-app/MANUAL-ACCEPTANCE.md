...
4. Bind an Active ChangeSet to one conversation manually, restart the Java app and require the binding to remain. Binding must not automatically send the already-current ReviewDiff.
   - For a different unbound ChangeSet, Apply an `OBS-ACTION/1` containing `chatTabTitle` exactly equal to one currently open ordinary ChatGPT conversation title; require the app to create the same persisted Review-chat binding through the normal binding path and queue that Apply's current ReviewDiff.
   - Repeat with no matching title and with two different conversations sharing that exact title; require Apply to remain successful, no destination to be guessed, an actionable manual-binding warning, and the normal manual binding controls to remain usable.
   - With a pre-existing manual binding, Apply a continuation whose action carries a different matching title; require the existing binding to remain authoritative and the new ReviewDiff to queue to that existing conversation.
5. Click `Send current ReviewDiff` with an empty composer and a small diff; require one exact `.diff` attachment whose browser-visible filename contains the delivery task identity to appear, become upload-ready and then be sent to the bound conversation. No ReviewDiff text should be injected into the rich-text editor. Confirm the resulting outgoing user turn contains a file/attachment-like DOM surface exposing `.diff` somewhere in the complete turn container, even when that file card is not a descendant of `[data-message-author-role="user"]`; do not use ordinary message text as proof. Record the actual turn/file-card element and whether the live DOM also retains the full untruncated task-specific filename so exact post-Send identity can be strengthened later.
...
18. With **no active in-flight delivery**, reload the unpacked extension while ordinary ChatGPT tabs are already open; require the service worker to inject/reconnect the content script without requiring a manual tab refresh. Runtime-generation continuity for an already claimed/in-flight task during reload/update/close is a known deferred risk and is not an acceptance gate in this campaign.

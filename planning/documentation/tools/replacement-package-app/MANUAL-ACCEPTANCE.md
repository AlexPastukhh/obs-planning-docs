...
1. Load `chatgpt-bridge-extension/` unpacked in Microsoft Edge, start the Java app, copy the pairing token from the app, save it in extension Options and require the connection test to pass.
   - Confirm Options reports protocol `4`. Then intentionally run/reconnect an older Java bridge that does not advertise protocol `4`; require an actionable version mismatch and `FailedBeforeSend` before attachment/composer mutation. Restart the current app and reload the extension before continuing.

...
11. While a task-specific ReviewDiff attachment is still prepared, create an unrelated outgoing user turn and require it **not** to confirm the task as `Sent`; only a post-baseline outgoing turn carrying this task-specific filename may confirm delivery. Force the ReviewDiff attachment to disappear **before** any automatic click and require `PreparedUnsent`. Separately force it to disappear after an actual possible-Send click without observing the task-specific outgoing turn; require `UnknownAfterSend` and **no further automatic Send attempts**. Then close all tabs for a separately queued bound interaction and require it to stay pending without changing Apply/Review success; reopen the conversation and require delivery to resume.

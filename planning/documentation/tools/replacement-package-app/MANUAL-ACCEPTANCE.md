...
Verify ordinary Apply and `Apply (wait for ZIP)` remain responsive, resolve exact ChangeSet/Repository Target, reject wrong repository/ownership/dirty-unowned/source-changed/unverifiable/readiness cases before mutation, accept Git-equivalent path representation, perform exact add/replace/delete, and preserve truthful rollback/divergence behavior. Verify wait-for-ZIP freezes click-time inputs, retries only package-not-found and enters the normal prepared Apply path once.

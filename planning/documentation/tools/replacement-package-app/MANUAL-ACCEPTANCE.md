...
5. Click `Send current ReviewDiff` with an empty composer and a small diff; require one exact `.diff` attachment to appear, become upload-ready and then be sent to the bound conversation. No ReviewDiff text should be injected into the rich-text editor.

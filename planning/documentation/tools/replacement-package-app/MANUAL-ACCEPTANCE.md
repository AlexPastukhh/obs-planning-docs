...
4. Bind an Active ChangeSet to one conversation manually, restart the Java app and require the binding to remain. Binding must not automatically send the already-current ReviewDiff.
   ...
   - Exercise both race orders. When token resolution reaches Java before the successful Apply ReviewDiff cutoff, require Chat A to become the normal persisted Review chat and that current ReviewDiff to queue. When Apply wins the race, require Apply SUCCESS plus a separate “ReviewDiff not sent — chat binding” Output/Windows notification, no current ReviewDiff queue, then a later “Review chat bound” notification when Chat A resolves; confirm that late success does not retroactively send the skipped ReviewDiff and that a later Refresh Review/Apply uses the binding.
   - Invoke ordinary `давай архив` after the bound invocation and require its action to omit `chatContextToken`; no new token lookup occurs and the existing persisted binding remains. Invoke `Bind + ...` again and require a new token. A token resolving to a different already-bound conversation must not silently rebind.

...
**Target property:** Local/Committed repository context is exported as the documented stable ZIP without mutating repository work/index; the same dialog makes `Export only`, destination-first `Export + Attach`, and destination-first `Export + Attach + Send` explicit. Export-only retains the post-export path/copy/open-folder result dialog, while either automatic ChatGPT handoff suppresses that second modal; artifact success survives downstream clipboard/browser problems.  
**Execution state:** `planned`

1. Select a registered repository and click `Export repository ZIP`. Require one Repository Snapshot dialog with Mode, destination, a ChatGPT conversation selector, and `Export only` / `Export + Attach` / `Export + Attach + Send` / Cancel. There is no separate attach toggle and no post-export chat-selection dialog.
...
17. With no ChatGPT conversation currently visible, require `Export only` to remain usable. Choosing either `Export + Attach` or `Export + Attach + Send` must not start export without a selected conversation; open the intended conversation and reopen the Snapshot dialog instead of creating an ambiguous/no-destination interaction.

## `PA-SL05` — Attach Repository Snapshot To ChatGPT

**Target property:** the exact validated Repository Snapshot follows the explicitly selected initial handoff mode: export-only manual path, attach-only prepared composer, or attach+Send through the generic delivery engine.
**Execution state:** `planned`

1. Open the Repository Snapshot dialog while at least two ordinary ChatGPT conversations are visible. Require distinct `Export only`, `Export + Attach`, and `Export + Attach + Send` choices. Select Chat A before export; no second destination chooser may appear.
...
3. With `Export + Attach`, require the exact ZIP filename to become ready in Chat A only, one Snapshot External Interaction with exact destination, terminal `Attached`, and no Send click. Also require no second path/copy modal after export.
4. Repeat `Export + Attach` with Chat A unavailable during/after export. Export must remain successful with no substitution. Known-missing enqueue fails downstream; stale-inventory queued work reaches the fixed `Cancelled`/`PreparedUnsent` deadline truth. Reopen and explicitly retry if desired.
5. Repeat with `Export + Attach + Send`. Require the same generic browser attachment primitive and guarded MAIN-world Send path used by ReviewDiff, clean composer before mutation, exact ZIP filename present, Java `SendArmed` authorization before the first application-controlled MAIN-world click, actual possible click before `SendClicked`, then `Sent` or truthful `UnknownAfterSend`. Make that first click ineffective while the same ZIP attachment remains prepared and require a later guarded retry to occur from the existing `SendClicked` lifecycle without a second `SendArmed` transition. Require the Snapshot confirmation deadline not to fire between authorization and the first browser click; a definitive no-click before any possible click must restore the original absolute deadline. No second path/copy modal may appear.
...
1. Load `chatgpt-bridge-extension/` unpacked in Microsoft Edge, start the Java app, copy the pairing token from the app, save it in extension Options and require the connection test to pass.
   - Confirm Options reports protocol `5`. Then intentionally run/reconnect an older Java bridge that does not advertise protocol `5`; require an actionable version mismatch and `FailedBeforeSend` before attachment/composer mutation. Restart the current app and reload the extension before continuing.
...
4. Bind an Active ChangeSet to one conversation manually, restart the Java app and require the binding to remain. Binding must not automatically send the already-current ReviewDiff.
   ...
   - From Planning Helper in Chat A use an explicit `Bind + ...` `давай архив` invocation. Require its `OBS-ACTION/1` to contain that invocation's exact `chatContextToken`. Start Apply while Chat A remains open and verify extension protocol 5 asks live agents for the token capture rather than matching the title.
   - Exercise both race orders. When token resolution reaches Java before the successful Apply ReviewDiff cutoff, require the captured chat to become the persisted Review chat immediately (binding or rebinding as needed) and that current ReviewDiff to queue there. When Apply wins the race, require Apply SUCCESS plus a separate “ReviewDiff not sent — chat binding” Output/Windows notification, no current ReviewDiff queue, then a later “Review chat bound” / “Review chat rebound” notification when resolution occurs; confirm that late success does not retroactively send the skipped ReviewDiff and that a later Refresh Review/Apply uses the new binding.
   - Invoke ordinary `давай архив` after the bound invocation and require its action to omit `chatContextToken`; no new token lookup occurs and the existing persisted binding remains. Invoke `Bind + ...` in a different Chat B and require a new token; when it resolves, require the persisted Review chat to rebind from A to B immediately with no second confirmation. Repeat with repository Apply intentionally failing and require the token lookup/bind-rebind outcome to remain independent of that failure.
...
11. While a task-specific ReviewDiff attachment is still prepared, create an unrelated outgoing user turn and require it **not** to confirm the task while the exact prepared attachment is still present. Then perform a real automatic Send and require `Sent` once that prepared attachment has left the composer and a new post-baseline user turn exists, even when ChatGPT does not expose a stable `.diff` card in that turn. When a `.diff` file-card is exposed, verify the stronger proof remains bounded to the current authored turn and cannot be borrowed from a neighboring turn. Force the ReviewDiff attachment to disappear **before** any automatic click and require `PreparedUnsent`. Separately force it to disappear after an actual possible-Send click with **no** post-baseline user turn; require `UnknownAfterSend` and **no further automatic Send attempts**. Then close all tabs for a separately queued bound interaction and require it to stay pending without changing Apply/Review success; reopen the conversation and require delivery to resume.
...
**Target property:** current-change and snapshot handoffs that are still active/actionable or unacknowledged-uncertain appear as user-semantic External Interactions with exact source/destination, truthful Cancel behavior and separate terminal uncertainty Dismiss acknowledgement; ordinary terminal attempts do not accumulate as list history.  
**Execution state:** `implementation present / manual practical evidence pending`

1. Create one current-change delivery and one snapshot-attachment interaction; require both to appear in one list with distinguishable kind/source/destination/status. Require the interaction selector to use the full row and `Refresh interactions` / `Cancel interaction` / `Dismiss interaction` to remain visible directly below it even when the selected interaction text is long.
...
7. Force possible-Send uncertainty after `SendClicked`; require `UnknownAfterSend` truth to remain visible, no ability to rewrite it to Cancelled or auto-resend, and `Dismiss interaction` to remove only the working-list row. Restart the app and require the dismissed row to stay hidden while the persisted task/result remains `UnknownAfterSend`.
8. While a current-change interaction is still actionable, request the same current ReviewDiff for the same conversation again; require reuse of the existing interaction identity and exactly one Pending/actionable list row. Repeat with the same snapshot artifact + destination + same attach-only/auto-send intent. Switching that same Snapshot between attach-only and attach+Send is a materially different intent and must not reuse the other actionable interaction. After Cancel/ordinary terminal completion, explicitly retry the same user intent; require a **new External Interaction identity**, never restoration/reuse of the terminal one.

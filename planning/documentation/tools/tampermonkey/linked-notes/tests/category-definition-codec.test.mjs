...
test('category definition v2 round-trips literal description and visible links', () => {
  ...
  assert.match(markdown, /obs-file-category:v2/);
...
test('managed-looking headings remain literal description content in v2', () => {
...
test('legacy v1 definitions remain readable and re-encode deterministically as v2', () => {
  ...
  assert.match(upgraded, /obs-file-category:v2/);
  assert.equal(api.decodeCategoryDefinition(upgraded).description, 'Old description.');
});

test('malformed v2 managed boundaries are explicit', () => {

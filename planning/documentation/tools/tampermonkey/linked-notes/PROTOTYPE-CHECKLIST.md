...
| 4e | Resize the browser while Notes is open, first while centered, again after a normal custom drag and once while left/right edge-peeked. | Centered mode recomputes the center; normal custom mode keeps its position when possible; edge-peek remains recoverable on the same side with its visible strip; top/bottom remain bounded and controls are recoverable through the edge grip or `Center`. | |
| 4f | Drag `Linked Notes · drag` almost fully beyond the left edge and then the right edge; use the exposed edge recovery grip to pull it back; while the pointer is still held trigger a harmless UI/status rerender, continue the drag, collapse/reopen Docs, then press `Center`; repeat once with `Locations` or `Reference objects` open before drag and once before viewport resize. | Horizontal drag may intentionally hide most of the open panel while exactly a recoverable strip/grip remains visible; top/bottom cannot be hidden away; destructive rerender does not detach/reset the gesture; peek/custom position survives collapse/reopen in the same live runtime; `Center` restores full centered mode; a portaled top popup closes before movement/repositioning. | |
...
| 77 | Open `Locations` and `Reference objects`, activate an internal control in each, trigger a harmless Files rerender, then reopen and click the ChatGPT page outside the Linked Notes Shadow root; repeat with Escape. | Both portaled panels remain visibly above the main Linked Notes panel, inherit readable theme-consistent foreground text and remain clickable before/after rerender; document-level outside pointerdown and Escape close the shared popup without consuming the underlying page interaction. | |

...
function registryDirectionId(rel,directions){return directions.find((direction)=>(direction.sources||[]).includes(rel))?.id||'';}
...
function projectedItem(raw,rel,commands,directions){
  ...
  const directionId=cleanCell(raw['parent direction'])||registryDirectionId(rel,directions);if(!directionId)return null;
  ...
  const item={id,label,description:purpose||result||label,sources:[rel],instruction,target:`<${label} target>`,directionId,manualInvocation:true,trigger,result};if(commandId)item.commandId=commandId;return semantic.normalizeUseCaseDefinition(item);
}
function readCanonicalUseCases(commands,directions){
  ...
  for(const file of walk(path.join(repoRoot,'planning')).sort()){
    ...
    for(const line of lines){if(/^\|\s*ID\s*\|/i.test(line)){headers=splitRow(line).map((x)=>cleanCell(x).toLowerCase());continue;}if(!headers||!/^\|\s*`UC-[A-Z0-9-]+`\s*\|/.test(line))continue;const cells=splitRow(line),row={};headers.forEach((h,i)=>row[h]=cells[i]||'');add(projectedItem(row,rel,commands,directions),rel);}
    let section=null;const flush=()=>{if(section){add(projectedItem(section,rel,commands,directions),rel);section=null;}};
...
function seedText(kind,items){const generatedFrom=kind==='planning-command-seed'?'planning/commands/*.command.md':kind==='use-case-seed'?'all current canonical Use-Case registries under planning/** (case-insensitive filename; legacy/historical compatibility indexes excluded; root aggregation may repeat scoped entries)':'planning/direction-registry.md';return JSON.stringify({schemaVersion:1,kind,generatedFrom,items},null,2)+'\n';}

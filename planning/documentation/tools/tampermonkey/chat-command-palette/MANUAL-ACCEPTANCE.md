...
- Reorder Commands/Scenarios, move a Command with **Group**, run `Save order GitHub`, and verify only `catalog-order.json` changes with intended stable IDs/group membership.
...
- [ ] Commands navigation keeps the top-level classifications `General`, `Use Cases`, `Target Modules`, `Lenses`, `Tools / Repository` and renders smaller GitHub-backed presentation groups inside each classification.
...
- [ ] `Group` changes only local presentation grouping and `Save order GitHub` persists it through `catalog-order.json`.

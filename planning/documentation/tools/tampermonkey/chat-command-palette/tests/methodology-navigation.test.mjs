...
test('command navigation is derived from semantic identity and exposes UC/TM/Lens as classifications',()=>{
  ...
  assert.equal(navigation.methodologyPrimaryIds(entries,'TARGET_MODULES').length,30);
...
test('every command card belongs to one normal tab/group and All commands is only a cross-tab projection',()=>{
  ...
  assert.deepEqual(views.map((view)=>[view.id,view.count]),[['GENERAL',20],['IDTSPE_PASS',38],['USE_CASES',19],['TARGET_MODULES',30],['LENSES',27],['TOOLS',4]]);

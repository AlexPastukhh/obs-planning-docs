...
test('important current Use Cases and their standalone command routes remain independently discoverable',()=>{for(const id of ['UC-REPO-AUDIT-REVIEW','UC-PLAN-ARCH-WORKSPACE-USES','UC-PLAN-DOMAIN','UC-PLAN-SLICE','UC-PLAN-TEST-PLAN','UC-PLAN-ORIENT'])assert.ok(useCases.some((u)=>u.id===id),id);for(const file of ['review-audit.command.md','discover-workspace-use-cases.command.md','plan-domain.command.md','plan-application-slice.command.md','plan-practical-testing.command.md','bootstrap-application-sds-planning.command.md'])assert.ok(fs.existsSync(path.join(repoRoot,'planning/commands',file)),file)});
...
test('semantic runtime source contains no maintained current UC catalog identities or Direction ontology',()=>{const source=fs.readFileSync(path.join(moduleRoot,'src/semantic-projections.js'),'utf8');for(const identity of ['UC-PLAN-DOMAIN','UC-REPO-CURRENT-STATE','DIR-PLAN-SOLUTION','application_domain.plan'])assert.doesNotMatch(source,new RegExp(identity.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));assert.doesNotMatch(source,/normalizeDirectionDefinitions|directionId|directionIds/);assert.match(source,/normalizeUseCaseDefinitions/)});

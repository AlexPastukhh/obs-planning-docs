...
const d=(id,file,command,aliases=[command],includes=[])=>({id,file,command,commandFamily:aliases,palette:true,includes});
...
test('validates registered command include graph',()=>{const a=d('a','a.command.md','a',['a'],['b']),b=d('b','b.command.md','b');assert.equal(catalog.validateCommandCatalog([a,b]).byId.get('a').includes[0],'b');});
test('rejects unknown included command in complete catalogs',()=>assert.throws(()=>catalog.validateCommandCatalog([d('a','a.command.md','a',['a'],['missing'])]),/Unknown included command id missing/));
test('allows missing included command in partial local projections',()=>assert.equal(catalog.validateCommandCatalog([d('a','a.command.md','a',['a'],['missing'])],{allowMissingIncludes:true}).definitions.length,1));
test('rejects command include cycles',()=>assert.throws(()=>catalog.validateCommandCatalog([d('a','a.command.md','a',['a'],['b']),d('b','b.command.md','b',['b'],['a'])]),/Command include cycle/));

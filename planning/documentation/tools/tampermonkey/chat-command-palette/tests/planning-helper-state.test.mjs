...
test('warm snapshot receives new bundled commands unless that command was locally deleted',async()=>{const existing=state.normalizePlanningHelperLocalSnapshot({schemaVersion:2,savedAt:'2026-01-01T00:00:00Z',planningCommands:[{definition:def,repositoryKnown:true}],helperItems:[],hiddenCommandIds:['hidden.command'],hiddenUseCaseIds:[]});const added={...def,id:'new.command',file:'new.command.command.md',command:'new command',englishName:'new command',commandFamily:['new command']};const hidden={...def,id:'hidden.command',file:'hidden.command.command.md',command:'hidden command',englishName:'hidden command',commandFamily:['hidden command']};await withGm({[state.PLANNING_HELPER_STATE_KEYS.localSnapshot]:existing},async()=>{const result=await state.loadOrMigratePlanningHelperLocalSnapshot([def,added,hidden]);assert.equal(result.seededCommands,1);assert.deepEqual(result.snapshot.planningCommands.map((r)=>r.definition.id).sort(),['demo.create','new.command']);assert.deepEqual(result.snapshot.hiddenCommandIds,['hidden.command']);});});

test('schema-1 local snapshot migrates to schema 2 without losing content',async()=>{const legacy={schemaVersion:1,savedAt:'2026-01-01T00:00:00Z',planningCommands:[{definition:def,repositoryKnown:true}],helperItems:[]};await withGm({[state.PLANNING_HELPER_STATE_KEYS.localSnapshot]:legacy},async()=>{const result=await state.loadOrMigratePlanningHelperLocalSnapshot([def]);assert.equal(result.migrated,true);assert.equal(result.snapshot.schemaVersion,2);assert.deepEqual(result.snapshot.hiddenCommandIds,[]);assert.deepEqual(result.snapshot.hiddenUseCaseIds,[]);});});

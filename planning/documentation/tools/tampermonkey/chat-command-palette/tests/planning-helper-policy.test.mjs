...
test('Planning Helper has canonical application registry and Scenario owners',()=>{const registry=fs.readFileSync(path.join(root,'scenarios/README.md'),'utf8');const catalog=fs.readFileSync(path.join(root,'scenarios/README.md'),'utf8');for(const id of ['SCN-PH-DISCOVER','SCN-PH-USE','SCN-PH-MANAGE-LOCAL','SCN-PH-IMPORT','SCN-PH-CHECK-REPOSITORY','SCN-PH-SYNC','SCN-PH-PUBLISH','SCN-PH-RECOVER']){assert.match(registry,new RegExp(id));assert.match(catalog,new RegExp(id))}const scenario=fs.readFileSync(path.join(root,'scenarios/SCN-PH-DISCOVER.md'),'utf8');assert.match(scenario,/Primary implementation/);assert.match(scenario,/Automated evidence/);assert.match(scenario,/Manual acceptance/)});

test('root semantic docs point to Planning Helper Direction and Scenario Catalog',()=>{const directions=fs.readFileSync(path.join(repoRoot,'planning/direction-registry.md'),'utf8');assert.match(directions,/chat-command-palette\/direction-registry\.md/);assert.match(directions,/chat-command-palette\/scenarios\/README\.md/)});
...
test('semantic projections expose current planning UCs and omit retired planning identities',()=>{const source=read('src/semantic-projections.js');for(const id of ['UC-PLAN-COLLECT-IDEAS','UC-PLAN-APP-CONCEPT','UC-PLAN-PROTOTYPE','UC-PLAN-DOMAIN','UC-PLAN-SLICE-STRATEGY'])assert.match(source,new RegExp(id));assert.match(source,/ideas\.collect/);assert.doesNotMatch(source,/planning_items\.form/);assert.doesNotMatch(source,/planning_items\.reconcile/);assert.doesNotMatch(source,/UC-AP-FORM-ITEMS/);assert.doesNotMatch(source,/UC-AP-RECONCILE/);assert.match(source,/route_resolution/);assert.match(source,/Semantic planning\/read context only/)});

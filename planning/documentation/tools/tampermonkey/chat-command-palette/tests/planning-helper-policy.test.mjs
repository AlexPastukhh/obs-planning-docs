...
test('semantic projections expose current application planning UCs and retire old Planning Item/Spine identities',()=>{const source=read('src/semantic-projections.js');for(const id of ['UC-PLAN-COLLECT-IDEAS','UC-PLAN-APP-CONCEPT','UC-PLAN-PROTOTYPE','UC-PLAN-DOMAIN','UC-PLAN-SLICE-STRATEGY'])assert.match(source,new RegExp(id));assert.match(source,/ideas\.collect/);assert.doesNotMatch(source,/UC-PLAN-SPINE/);assert.doesNotMatch(source,/planning_items\.form/);assert.doesNotMatch(source,/planning_items\.reconcile/);assert.doesNotMatch(source,/UC-AP-FORM-ITEMS/);assert.doesNotMatch(source,/UC-AP-RECONCILE/);assert.match(source,/route_resolution/);assert.match(source,/Semantic planning\/read context only/)});

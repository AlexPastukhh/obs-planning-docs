...
// @version      0.31.0-repository-command-registry
...
@media(max-width:760px){.panel{min-width:0;width:calc(100vw - 16px)!important;left:8px!important}.row{grid-template-columns:1fr}.actions{justify-content:flex-start;flex-wrap:wrap}.direction-entries{padding-left:7px}}
    ...
    function applyState(result={}){if(result.commandEntries)commandEntries=[...result.commandEntries];if(result.promptEntries)promptEntries=[...result.promptEntries];if(result.useCaseEntries)useCaseEntries=[...result.useCaseEntries];if(result.directionDefinitions)directionDefinitions=[...result.directionDefinitions];if(result.favoriteCommandIds)favoriteCommandIds=[...result.favoriteCommandIds];if(result.favoriteUseCaseIds)favoriteUseCaseIds=[...result.favoriteUseCaseIds];renderEntries(searchInput.value);}
    ...
    function switchSurface(surface){activeSurface=surface;for(const button of tabButtons)button.setAttribute('aria-selected',String(button.dataset.surface===surface));newLibraryButton.style.display=surface===SURFACES.USE_CASES?'none':'inline-block';searchInput.value='';renderEntries('');}
    ...
    function makeEntryRow(entry,{favorite=false}={}){const row=document.createElement('div');row.className='row';const insert=document.createElement('button');insert.type='button';insert.className='insert';insert.innerHTML=`<span class="row-label"></span><span class="row-meta"></span>`;insert.querySelector('.row-label').textContent=entry.label||entry.title||entry.command||entry.id;insert.querySelector('.row-meta').textContent=[entry.description,entry.stateLabel].filter(Boolean).join(' · ');insert.addEventListener('click',()=>insertBody(entry.adaptiveBody||entry.text,`Inserted ${entry.label||entry.id}`,entry.id));row.append(insert);const actions=document.createElement('div');actions.className='actions';

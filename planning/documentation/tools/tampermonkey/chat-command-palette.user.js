...
// @version      0.35.0-repository-command-registry
...
(function (root, factory) {
  ...
  // Compatibility fallbacks are read-only: old helperPresentation can still be understood
  // in imported legacy records, but current navigation is derived from semantic identity.
...
@media(max-width:760px){.panel{min-width:0;width:calc(100vw - 16px)!important;left:8px!important}.row{grid-template-columns:1fr}.actions{justify-content:flex-start;flex-wrap:wrap}.entry-entries{padding-left:7px}}
    ...
    function renderMethodologyGroups(groups,needle){const activeView=currentMethodologyViews().find((view)=>view.id===activeMethodologyView);for(const group of groups){const entries=needle?group.entries.filter((entry)=>JSON.stringify([entry.id,entry.label,entry.description,entry.command,entry.__methodologyNav?.badges]).toLowerCase().includes(needle)):group.entries;if(!entries.length)continue;const details=document.createElement('details');details.className='entry-group';details.open=true;const head=document.createElement('div');head.className='entry-head';const summary=document.createElement('summary');summary.innerHTML='<span></span><small class="entry-meta"></small>';summary.querySelector('span').textContent=group.label;summary.querySelector('.entry-meta').textContent=activeView?`${activeView.label} navigation`:'Methodology navigation';head.append(summary);details.append(head);const rows=document.createElement('div');rows.className='entry-entries';for(const entry of entries)rows.append(makeEntryRow(entry));details.append(rows);body.append(details);}}
    ...
    function hardReloadRepository(){const{overlay,modal}=makeOverlay('Hard Reload GitHub');const p=document.createElement('p');p.textContent='Replace the complete local Planning Command, semantic-component and Scenario catalogs with current GitHub-backed content, restore locally hidden catalog rows, and replace local catalog order with GitHub catalog-order.json. Local prompt content is not replaced. Unsaved local command edits will be lost.';const actions=document.createElement('div');actions.className='modal-actions';actions.append(button('Cancel','',()=>closeOverlay(overlay)),button('Hard Reload','danger',async()=>{operationBusy=true;setBusy();try{const result=await options.onHardReloadRepository();applyState(result);closeOverlay(overlay);showStatus(`Hard Reload complete: ${result.commands} direct definitions, ${result.semanticComponents} semantic components, ${result.scenarios} scenarios. GitHub order loaded.`,9000);}catch(error){showStatus(`Hard Reload failed: ${error.message||String(error)}`,9000);}finally{operationBusy=false;setBusy();}}));modal.append(p,actions);}

...
(function (root, factory) {
  ...
  return{SURFACES,MODE,normalizeUseCaseDefinition,normalizeUseCaseDefinitions,buildSemanticBody,buildSemanticEntries};
});
...
(function (root, factory) {
  ...
  return{USE_CASE_CATALOG_PATH,CATALOG_ORDER_PATH,CATALOG_ORDER_KIND,normalizeCatalogOrder,renderCatalogOrder,parseUseCaseCatalog,parseCatalogOrder,RepositoryCatalogService};
});
...
(function (root, factory) {
  ...
  const LOCAL_SNAPSHOT_SCHEMA_VERSION=5;
  ...
  function normalizePlanningHelperLocalSnapshot(value){
    if(!value||typeof value!=='object'||![1,2,3,4,LOCAL_SNAPSHOT_SCHEMA_VERSION].includes(value.schemaVersion))throw new TypeError('Unsupported Planning Helper local snapshot schema.');
    ...
    return{schemaVersion:LOCAL_SNAPSHOT_SCHEMA_VERSION,savedAt:cleanIso(value.savedAt,''),planningCommands,helperItems,useCases,useCaseCatalogSha,catalogOrder,catalogOrderSha,hiddenCommandIds,hiddenUseCaseIds,favoriteCommandIds,favoriteUseCaseIds};
  }
  ...
  async function loadOrMigratePlanningHelperLocalSnapshot(){
    ...
    if(existingRaw!=null){const existing=normalizePlanningHelperLocalSnapshot(existingRaw),needsWrite=existingRaw.schemaVersion!==LOCAL_SNAPSHOT_SCHEMA_VERSION,snapshot=needsWrite?await savePlanningHelperLocalSnapshot(existing):existing;if(needsWrite&&!existing.useCases.length)warnings.push('Planning Helper local snapshot migrated. Commands and Use Cases are GitHub-backed; use Hard Reload GitHub to restore current repository catalogs.');return{snapshot,migrated:needsWrite,seededCommands:0,warnings};}
    let definitions=[];
    ...
    const snapshot=await savePlanningHelperLocalSnapshot({schemaVersion:LOCAL_SNAPSHOT_SCHEMA_VERSION,planningCommands:commandRecordsFromDefinitions(definitions,true),helperItems:[...helperByKey.values()],useCases:[],useCaseCatalogSha:'',catalogOrder:deps.normalizeCatalogOrder({}),catalogOrderSha:'',hiddenCommandIds:[],hiddenUseCaseIds:[],favoriteCommandIds:[],favoriteUseCaseIds:[]});
    warnings.push('Commands and Use Cases are repository-backed catalogs. Use Hard Reload GitHub to populate/restore them from the configured repository.');
...
@media(max-width:760px){.panel{min-width:0;width:calc(100vw - 16px)!important;left:8px!important}.row{grid-template-columns:1fr}.actions{justify-content:flex-start;flex-wrap:wrap}.entry-entries{padding-left:7px}}
    ...
    async function syncMissingRepository(){operationBusy=true;setBusy();try{showStatus('Syncing GitHub-only records…',9000);const result=await options.onSyncMissingRepository();applyState(result);showStatus(`Sync complete: ${result.addedCommands||0} command(s), ${result.addedUseCases||0} Use Case(s), ${result.addedPrompts||0} prompt(s). Existing local same-ID/path records were not overwritten.`,9000);}catch(error){showStatus(`GitHub sync failed: ${error.message||String(error)}`,9000);}finally{operationBusy=false;setBusy();}}
    function hardReloadRepository(){const{overlay,modal}=makeOverlay('Hard Reload GitHub');const p=document.createElement('p');p.textContent='Replace the complete local Planning Command and Use-Case catalogs with current GitHub-backed content, restore locally hidden catalog rows, and replace local catalog order with GitHub catalog-order.json. Local prompt content is not replaced. Unsaved local command edits will be lost.';const actions=document.createElement('div');actions.className='modal-actions';actions.append(button('Cancel','',()=>closeOverlay(overlay)),button('Hard Reload','danger',async()=>{operationBusy=true;setBusy();try{const result=await options.onHardReloadRepository();applyState(result);closeOverlay(overlay);showStatus(`Hard Reload complete: ${result.commands} Commands, ${result.useCases} Use Cases. GitHub order loaded.`,9000);}catch(error){showStatus(`Hard Reload failed: ${error.message||String(error)}`,9000);}finally{operationBusy=false;setBusy();}}));modal.append(p,actions);}
...
(function (root, factory) {
  ...
  function materializeSnapshot(snapshot){
    const commandRecords=[...(snapshot.planningCommands||[])],helperRecords=[...(snapshot.helperItems||[])],useCases=deps.normalizeUseCaseDefinitions(snapshot.useCases||[]),order=deps.normalizeCatalogOrder(snapshot.catalogOrder||{});
    ...
    return{commandRecords,helperRecords,useCases,order,commandByFile,commandById,helperByKey,commandEntries,localCommandEntries:sortByIds(legacyCommandEntries,order.commands),promptEntries,useCaseEntries:orderedUseCases};
  }
  ...
  function clearRepositoryEvidence(snapshot){const memory=materializeSnapshot(snapshot);return deps.normalizePlanningHelperLocalSnapshot({...snapshot,planningCommands:memory.commandRecords.map((record)=>deps.normalizeCommandRecord({...record,repositoryKnown:false,repositoryTracked:false,repositorySha:''})),helperItems:memory.helperRecords.map((record)=>deps.normalizeHelperRecord({...record,repositoryKnown:false,repositorySha:''})),useCaseCatalogSha:'',catalogOrderSha:''});}
  ...
  async function startPlanningHelper(){
    ...
    function uiState(){return{commandEntries:memory.commandEntries,localCommandEntries:memory.localCommandEntries,promptEntries:memory.promptEntries,useCaseEntries:memory.useCaseEntries,catalogOrder:memory.order,favoriteCommandIds:[...(snapshot.favoriteCommandIds||[])],favoriteUseCaseIds:[...(snapshot.favoriteUseCaseIds||[])]};}

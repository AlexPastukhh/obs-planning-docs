...
// @version      0.37.0-repository-command-registry
...
(function (root, factory) {
  ...
  const CATALOG_ORDER_SCHEMA_VERSION=3;
  const COMMAND_GROUP_LEVELS=Object.freeze({PRIMARY:'PRIMARY',ADVANCED:'ADVANCED',SEMANTIC_COMPONENT:'SEMANTIC_COMPONENT'});
  ...
  return{USE_CASE_CATALOG_PATH,SEMANTIC_COMPONENT_CATALOG_PATH,SCENARIO_CATALOG_PATH,CATALOG_ORDER_PATH,CATALOG_ORDER_KIND,CATALOG_ORDER_SCHEMA_VERSION,COMMAND_GROUP_LEVELS,normalizeCatalogOrder,renderCatalogOrder,parseUseCaseCatalog,parseSemanticComponentCatalog,parseScenarioCatalog,parseCatalogOrder,RepositoryCatalogService};
});
...
(function (root, factory) {
  ...
  const LOCAL_SNAPSHOT_SCHEMA_VERSION=6;
  ...
  function normalizePlanningHelperLocalSnapshot(value){
    if(!value||typeof value!=='object'||![1,2,3,4,5,LOCAL_SNAPSHOT_SCHEMA_VERSION].includes(value.schemaVersion))throw new TypeError('Unsupported Planning Helper local snapshot schema.');
...
@media(max-width:760px){.panel{min-width:0;width:calc(100vw - 16px)!important;left:8px!important}.row{grid-template-columns:1fr}.actions{justify-content:flex-start;flex-wrap:wrap}.entry-entries{padding-left:7px}}
    ...
    function groupLevelLabel(level){return deps.levelLabel?deps.levelLabel(level):String(level||'Primary').replaceAll('_',' ');}
    function openGroupPicker(entry){const viewId=deps.semanticNavigation?deps.semanticNavigation(entry).viewId:'GENERAL',groups=commandGroupsForEntry(entry),{overlay,modal}=makeOverlay(`Группа · ${entry.actionLabel||entry.label||entry.id}`),note=document.createElement('p');note.textContent='Grouping is presentation-only. Save order GitHub persists membership; Other / Ungrouped keeps cards visible when a group is removed. Hard Reload replaces local grouping with GitHub catalog-order.json.';modal.append(note);const actions=document.createElement('div');actions.className='modal-actions';const assign=async(groupId)=>{try{const result=await options.onAssignCatalogGroup(entry.id,groupId,viewId);applyState(result);closeOverlay(overlay);showStatus(groupId?'Group changed locally; use Save order GitHub to persist it.':'Moved to Other / Ungrouped locally; use Save order GitHub to persist it.',7000);}catch(error){showStatus(error.message||String(error),7000);}};actions.append(button('Other / Ungrouped','group-action',()=>assign('')));for(const group of groups)if(group.label!=='Other / Ungrouped')actions.append(button(`${groupLevelLabel(group.level)} · ${group.label}`,'group-action',()=>assign(group.id)));actions.append(button('Cancel','',()=>closeOverlay(overlay)));modal.append(actions);}
    function levelSelect(value){const select=document.createElement('select');for(const level of ['PRIMARY','ADVANCED','SEMANTIC_COMPONENT']){const option=document.createElement('option');option.value=level;option.textContent=groupLevelLabel(level);option.selected=level===String(value||'PRIMARY').toUpperCase();select.append(option);}return select;}
    ...
    function openGroupManager(){const viewId=activeGroupView(),view=currentMethodologyViews().find((item)=>item.id===viewId),{overlay,modal}=makeOverlay(`Manage groups · ${view?.label||viewId}`),note=document.createElement('p');note.textContent='Create, rename, classify and reorder presentation-only groups. Deleting a group moves its cards to Other / Ungrouped. Changes are local until Save order GitHub; Hard Reload replaces them from GitHub.';modal.append(note);for(const group of commandGroupsForView(viewId)){const row=document.createElement('div');row.className='group-editor';const name=document.createElement('input');name.value=group.label;name.disabled=group.label==='Other / Ungrouped';const level=levelSelect(group.level);const actions=document.createElement('div');actions.className='group-editor-actions';actions.append(button('Save','group-action',async()=>{try{applyState(await options.onUpdateCatalogGroup(group.id,{label:name.value,level:level.value}));closeOverlay(overlay);showStatus('Group updated locally; use Save order GitHub to persist it.',7000);}catch(error){showStatus(error.message||String(error),7000);}}),button('↑','group-action',async()=>{try{applyState(await options.onMoveCatalogGroup(group.id,-1));closeOverlay(overlay);showStatus('Group moved locally; use Save order GitHub to persist it.',7000);}catch(error){showStatus(error.message||String(error),7000);}}),button('↓','group-action',async()=>{try{applyState(await options.onMoveCatalogGroup(group.id,1));closeOverlay(overlay);showStatus('Group moved locally; use Save order GitHub to persist it.',7000);}catch(error){showStatus(error.message||String(error),7000);}}));if(group.label!=='Other / Ungrouped')actions.append(button('Delete','danger',async()=>{try{applyState(await options.onDeleteCatalogGroup(group.id));closeOverlay(overlay);showStatus('Group deleted locally; its cards moved to Other / Ungrouped. Use Save order GitHub to persist it.',8000);}catch(error){showStatus(error.message||String(error),7000);}}));row.append(name,level,actions);modal.append(row);}const h=document.createElement('h3');h.textContent='New group';const createRow=document.createElement('div');createRow.className='group-editor';const newName=document.createElement('input');newName.placeholder='Group name';const defaultLevel=['USE_CASES','TARGET_MODULES','LENSES'].includes(viewId)?'SEMANTIC_COMPONENT':'PRIMARY',newLevel=levelSelect(defaultLevel),createActions=document.createElement('div');createActions.className='group-editor-actions';createActions.append(button('Create','group-action',async()=>{try{applyState(await options.onCreateCatalogGroup({viewId,label:newName.value,level:newLevel.value}));closeOverlay(overlay);showStatus('Group created locally; use Save order GitHub to persist it.',7000);}catch(error){showStatus(error.message||String(error),7000);}}));createRow.append(newName,newLevel,createActions);const footer=document.createElement('div');footer.className='modal-actions';footer.append(button('Close','',()=>closeOverlay(overlay)));modal.append(h,createRow,footer);}
    ...
    function renderMethodologyGroups(groups,needle){const activeView=currentMethodologyViews().find((view)=>view.id===activeMethodologyView);for(const group of groups){const entries=needle?group.entries.filter((entry)=>JSON.stringify([entry.id,entry.label,entry.description,entry.command,entry.context,entry.result,entry.essence,entry.__methodologyNav?.badges]).toLowerCase().includes(needle)):group.entries;if(!entries.length)continue;const details=document.createElement('details');details.className='entry-group';details.open=true;const head=document.createElement('div');head.className='entry-head';const summary=document.createElement('summary');summary.innerHTML='<span></span><small class="entry-meta"></small>';summary.querySelector('span').textContent=group.label;summary.querySelector('.entry-meta').textContent=`${group.levelLabel||groupLevelLabel(group.level)} · ${activeView?activeView.label:'Methodology'} navigation`;head.append(summary);details.append(head);const rows=document.createElement('div');rows.className='entry-entries';for(const entry of entries)rows.append(makeEntryRow(entry));details.append(rows);body.append(details);}}
    ...
    function renderEntries(query=''){body.innerHTML='';const all=entriesForSurface(activeSurface),needle=String(query||'').trim().toLowerCase();if(activeSurface===SURFACES.COMMANDS&&activeMethodologyView!==ALL_METHODOLOGY_VIEW){const groups=deps.buildMethodologyViewGroups?deps.buildMethodologyViewGroups(all,activeMethodologyView):[];const count=groups.reduce((sum,group)=>sum+group.entries.filter((entry)=>!entry.__methodologyNav?.related).length,0);if(!count){const empty=document.createElement('div');empty.className='empty';empty.textContent='No methodology command rows. Use Hard Reload GitHub to restore current repository content.';body.append(empty);return;}renderMethodologyGroups(groups,needle);return;}const entries=needle?all.filter((entry)=>JSON.stringify([entry.id,entry.label,entry.title,entry.description,entry.command,entry.context,entry.result,entry.essence,entry.canonicalIntro,(entry.steps||[]).map((step)=>[step.title,step.canonicalText])]).toLowerCase().includes(needle)):all;if(!entries.length){const empty=document.createElement('div');empty.className='empty';empty.textContent=activeSurface===SURFACES.COMMANDS||activeSurface===SURFACES.SCENARIOS?'No local catalog rows. Use Hard Reload GitHub to restore current repository content.':'No items.';body.append(empty);return;}
      if(activeSurface===SURFACES.SCENARIOS){for(const entry of entries)body.append(makeScenarioRow(entry));return;}
      if(activeSurface===SURFACES.COMMANDS){const fav=favoriteEntries(entries,favoriteCommandIds);if(fav.length){const group=document.createElement('section');group.className='favorite-group';const title=document.createElement('div');title.className='favorite-title';title.textContent='★ Favorites';group.append(title);for(const entry of fav)group.append(makeEntryRow(entry,{favorite:true}));body.append(group);}for(const entry of entries)body.append(makeEntryRow(entry));return;}
...
(function (root, factory) {
  ...
  function presentationGroupForId(order,id){for(const group of order?.commandGroups||[]){const index=group.items.indexOf(String(id));if(index>=0)return{id:group.id,viewId:group.viewId,label:group.label,level:group.level,order:group.order,itemOrder:index};}return null;}
  ...
  function defaultGroupLevel(viewId){return['USE_CASES','TARGET_MODULES','LENSES'].includes(String(viewId||'').toUpperCase())?'SEMANTIC_COMPONENT':'PRIMARY';}

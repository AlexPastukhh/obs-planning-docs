...
// @version      0.39.0-repository-command-registry
...
(function (root, factory) {
  ...
  function mergeImportChatPatch(snapshot,parsed){const current=materializeSnapshot(snapshot),patch=parsed.patch||emptyPatch(),commandMap=new Map(current.commandRecords.map((record)=>[record.path,record])),helperMap=new Map(current.helperRecords.map((record)=>[record.path,record])),useCaseMap=new Map(current.useCases.map((entry)=>[entry.id,entry])),componentMap=new Map(current.semanticComponents.map((entry)=>[entry.id,entry])),scenarioMap=new Map(current.scenarios.map((entry)=>[entry.id,entry])),suppressed=suppressionState(snapshot),lines=[],newCommandRecords=[],newHelperRecords=[];let useCasesChanged=false,componentsChanged=false,scenariosChanged=false;
    for(const raw of patch.delete.commands||[]){const path=commandDeletePath(raw,current),record=commandMap.get(path);if(record){commandMap.delete(path);lines.push(`DELETE ${path}`);}else lines.push(`SUPPRESS ${path}`);suppressed.commands=withValue(suppressed.commands,path);if(record){snapshot={...snapshot,hiddenCommandIds:withoutValue(snapshot.hiddenCommandIds,record.definition.id),favoriteCommandIds:withoutValue(snapshot.favoriteCommandIds,record.definition.id)};}}
    for(const raw of patch.delete.helperItems||[]){const path=helperDeletePath(raw,current);if(helperMap.delete(path))lines.push(`DELETE ${path}`);else lines.push(`SUPPRESS ${path}`);suppressed.helperItems=withValue(suppressed.helperItems,path);}
    for(const raw of patch.delete.useCases||[]){const id=String(raw).trim();if(useCaseMap.delete(id)){useCasesChanged=true;lines.push(`DELETE use-case ${id}`);}else lines.push(`SUPPRESS use-case ${id}`);suppressed.useCases=withValue(suppressed.useCases,id);snapshot={...snapshot,hiddenUseCaseIds:withoutValue(snapshot.hiddenUseCaseIds,id),favoriteUseCaseIds:withoutValue(snapshot.favoriteUseCaseIds,id)};}
    for(const raw of patch.delete.semanticComponents||[]){const id=String(raw).trim();if(componentMap.delete(id)){componentsChanged=true;lines.push(`DELETE semantic-component ${id}`);}else lines.push(`SUPPRESS semantic-component ${id}`);suppressed.semanticComponents=withValue(suppressed.semanticComponents,id);}
    for(const raw of patch.delete.scenarios||[]){const id=String(raw).trim();if(scenarioMap.delete(id)){scenariosChanged=true;lines.push(`DELETE scenario ${id}`);}else lines.push(`SUPPRESS scenario ${id}`);suppressed.scenarios=withValue(suppressed.scenarios,id);}
    const commandUpserts=[...(parsed.definitions||[]),...(patch.upsert.commands||[])];
    for(const definitionValue of commandUpserts){const normalized=deps.normalizeCommandDefinition(definitionValue),path=deps.commandPathForDefinition(normalized),previous=current.commandByFile.get(normalized.file),idCollision=[...commandMap.values()].find((record)=>record.definition.id===normalized.id&&record.path!==path);if(idCollision)throw new TypeError(`Planning command id ${normalized.id} already belongs to ${idCollision.definition.file}.`);const rendered=deps.renderCommandDefinitionDocument(normalized),unchanged=Boolean(previous)&&previous.rawContent===rendered,record=deps.normalizeCommandRecord({definition:normalized,path,rawContent:rendered,repositoryKnown:unchanged&&Boolean(previous?.repositoryKnown),repositoryTracked:Boolean(previous?.repositoryTracked||previous?.repositoryKnown),repositorySha:unchanged?previous?.repositorySha||'':''});commandMap.set(path,record);suppressed.commands=withoutValue(suppressed.commands,path);snapshot={...snapshot,hiddenCommandIds:withoutValue(snapshot.hiddenCommandIds,normalized.id)};lines.push(`${previous?(unchanged?'UNCHANGED':'UPDATE'):'ADD'} ${path}`);if(!previous)newCommandRecords.push(record);}
    const helperUpserts=[...(parsed.helperItems||[]),...(patch.upsert.helperItems||[])];
    for(const itemValue of helperUpserts){const item=deps.normalizeHelperLibraryItem(itemValue),path=deps.helperLibraryTargetPath(item),previous=current.helperByKey.get(helperKey(item)),rendered=deps.renderHelperLibraryDocument(item),unchanged=Boolean(previous)&&previous.rawContent===rendered,record=deps.normalizeHelperRecord({item,path,rawContent:rendered,repositoryKnown:unchanged&&Boolean(previous?.repositoryKnown),repositorySha:unchanged?previous?.repositorySha||'':''});helperMap.set(path,record);suppressed.helperItems=withoutValue(suppressed.helperItems,path);lines.push(`${previous?(unchanged?'UNCHANGED':'UPDATE'):'ADD'} ${path}`);if(!previous)newHelperRecords.push(record);}
    for(const value of patch.upsert.useCases||[]){const item=deps.normalizeUseCaseDefinition(value),previous=current.useCases.find((entry)=>entry.id===item.id);useCaseMap.set(item.id,item);suppressed.useCases=withoutValue(suppressed.useCases,item.id);snapshot={...snapshot,hiddenUseCaseIds:withoutValue(snapshot.hiddenUseCaseIds,item.id)};const unchanged=Boolean(previous)&&sameJson(previous,item);if(!unchanged)useCasesChanged=true;lines.push(`${previous?(unchanged?'UNCHANGED':'UPDATE'):'ADD'} use-case ${item.id}`);}
    for(const value of patch.upsert.semanticComponents||[]){const item=deps.normalizeSemanticComponent(value),previous=current.semanticComponents.find((entry)=>entry.id===item.id);componentMap.set(item.id,item);suppressed.semanticComponents=withoutValue(suppressed.semanticComponents,item.id);const unchanged=Boolean(previous)&&sameJson(previous,item);if(!unchanged)componentsChanged=true;lines.push(`${previous?(unchanged?'UNCHANGED':'UPDATE'):'ADD'} semantic-component ${item.id}`);}
    for(const value of patch.upsert.scenarios||[]){const item=deps.normalizeScenario(value),previous=current.scenarios.find((entry)=>entry.id===item.id);scenarioMap.set(item.id,item);suppressed.scenarios=withoutValue(suppressed.scenarios,item.id);const unchanged=Boolean(previous)&&sameJson(previous,item);if(!unchanged)scenariosChanged=true;lines.push(`${previous?(unchanged?'UNCHANGED':'UPDATE'):'ADD'} scenario ${item.id}`);}
  ...
  function previewChatImport(snapshot,text,mode='import'){const parsed=deps.parseChatImport(text),merged=mergeChatImport(snapshot,parsed,mode);return{...merged,lines:merged.lines||[],mode};}

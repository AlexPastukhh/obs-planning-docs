...
// @version      0.39.10-repository-command-registry
...
(function (root, factory) {
  ...
  function materializeSnapshot(snapshot){
    ...
    for(const component of effectiveSemanticComponents){
      ...
      const compositionRootId=directDefinition?.id||(component.kind==='TARGET_MODULE'?'idtspe.target-module.apply':component.kind==='LENS'?'idtspe.lens.apply':null),effectiveCommandDag=(()=>{const out=[],seen=new Set(),byPath=new Map(definitions.map((node)=>[`planning/commands/${node.file}`,node]));function visit(node){if(!node||seen.has(node.id))return;seen.add(node.id);for(const path of node.includes||[])visit(byPath.get(path));out.push(`planning/commands/${node.file}`);}visit(commandById.get(compositionRootId));return out;})();

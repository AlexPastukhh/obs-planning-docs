...
// @version      0.36.0-repository-command-registry
...
@media(max-width:760px){.panel{min-width:0;width:calc(100vw - 16px)!important;left:8px!important}.row{grid-template-columns:1fr}.actions{justify-content:flex-start;flex-wrap:wrap}.entry-entries{padding-left:7px}}
    ...
    function commandGroupsForEntry(entry){const viewId=deps.semanticNavigation?deps.semanticNavigation(entry).viewId:'';return(catalogOrder?.commandGroups||[]).filter((group)=>group.viewId===viewId).sort((a,b)=>(Number(a.order)||0)-(Number(b.order)||0)||String(a.label).localeCompare(String(b.label)));}
    function openGroupPicker(entry){const groups=commandGroupsForEntry(entry),{overlay,modal}=makeOverlay(`Группа · ${entry.actionLabel||entry.label||entry.id}`),note=document.createElement('p');note.textContent='Grouping is presentation-only. Save order GitHub persists the updated group membership; Hard Reload GitHub replaces local grouping with current GitHub catalog-order.json.';modal.append(note);const actions=document.createElement('div');actions.className='modal-actions';const assign=async(groupId)=>{try{const result=await options.onAssignCatalogGroup(entry.id,groupId);applyState(result);closeOverlay(overlay);showStatus(groupId?'Group changed locally; use Save order GitHub to persist it.':'Removed from explicit group locally; use Save order GitHub to persist it.',7000);}catch(error){showStatus(error.message||String(error),7000);}};actions.append(button('Ungrouped','group-action',()=>assign('')));for(const group of groups)actions.append(button(group.label,'group-action',()=>assign(group.id)));actions.append(button('Cancel','',()=>closeOverlay(overlay)));modal.append(actions);}
...
(function (root, factory) {
  ...
  function assignCommandGroupInOrder(value,id,groupId){const order=deps.normalizeCatalogOrder(value||{}),itemId=String(id||'').trim(),targetId=String(groupId||'').trim();if(!itemId)throw new Error('Command card id is required.');let found=!targetId;order.commandGroups=order.commandGroups.map((group)=>{const items=group.items.filter((item)=>item!==itemId);if(group.id===targetId){found=true;items.push(itemId);}return{...group,items};});if(!found)throw new Error(`Command group not found: ${targetId}`);return deps.normalizeCatalogOrder(order);}
  ...
  async function startPlanningHelper(){
    ...
    async function assignCatalogGroup(id,groupId){const order=assignCommandGroupInOrder(snapshot.catalogOrder||{},id,groupId);return persist({...snapshot,catalogOrder:order,catalogOrderSha:''});}

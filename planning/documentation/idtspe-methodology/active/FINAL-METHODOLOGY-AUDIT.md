...
- Artifact Placement remains available through P-14 / TF-10 and can place meaning into code/existing sections/generated views as well as files, but ordinary Broad Discussion does not require a full Artifact Placement View; checkpoints/persistence-sensitive passes expose placement when material/changed/unresolved and may inherit it compactly when unchanged: **PASS**
...
## Broad Discussion / Integration Checkpoint Core Model — current state

- Broad Discussion is a first-class normal multi-turn exploratory workspace around the current Target/context and does not require a full Generic-State/Target-Result/Artifact-Placement dump in every reply: **PASS**
- the generic AI Reviewability Key Points contract still applies proportionally to material outputs; within Broad Discussion, Key Points structure material logical discussion parts and may summarize conclusions, alternatives, trade-offs or unresolved tension: **PASS**
- no mandatory block-owner/semantic-anchor record or per-response Broad Discussion Intake Summary exists; ordinary conversation context plus explicit material Ideas/Q-R-P/Evidence carry exploration until a whole-state checkpoint is useful: **PASS**
- every material Idea is explicitly surfaced and carries `Addresses → current Target Goal / Question / Problem`; the Target Goal / Desired Outcome is Target/scope context rather than a new Generic State Unit, and an unresolved driver is surfaced as a planning gap rather than a free-floating material Idea: **PASS**
- an active Target Module formally seeds decision-driver discovery with the current Target Goal plus reusable Question candidates and Problem candidates; Problem uses existing P-09 Problem semantics, while materially valid drivers/Ideas from Sources/situation, previous work/checkpoints, Broad Discussion, user, AI or dispositioned findings enter on equal Core semantic footing once accepted: **PASS**
- Ideas may compete, complement, require/conflict with one another and compose into lightweight Candidate Bundles / Option Groups; material option relations are shown when they matter, and bundle grouping is not a new required State Unit or Planning Branch: **PASS**
- Q/R/P/Evidence may be contextual to Target, Target Goal context, Questions/Problems, Ideas/bundles, Result Units/fields, relations/boundaries or Decisions rather than one undifferentiated Target-wide list: **PASS**
- Integration Checkpoint is a periodic Collect/Relate/Integrate/Check pass over accumulated discussion/current State; it renders Target Goal context + coherent existing Generic State + Target Result Units + relevant Lens/consistency review, preserves unresolved alternatives, and is not completion/approval/lifecycle status/Unit/Target/file/commit/new command: **PASS**
- checkpoint invocation does not end Broad Discussion; if checkpoint explanation itself surfaces new material Idea/Question/Problem/Evidence/Decision meaning, it is integrated into the same checkpoint or explicitly marked as post-checkpoint exploration: **PASS**
- optional checkpoint `Broad Discussion Summary` remains retrospective prose and never substitutes for explicit Generic State/Idea relations: **PASS**
- accepted material Decisions are retained in semantic planning state by default; optional `Selected` Idea/bundle trace and optional `Rationale / Why` are supported, with rationale explicitly distinct from Evidence: **PASS**
...
- canonical `idtspe.work`, Core command-surface contract and Helper seed expose the same corrected discussion/checkpoint semantics; generic Core command count remains 9: **PASS**

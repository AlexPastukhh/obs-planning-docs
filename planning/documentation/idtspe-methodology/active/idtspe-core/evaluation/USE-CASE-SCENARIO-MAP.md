...
**Derived From / Owners:** `planning/documentation/idtspe-methodology/active/idtspe-core/resolution/needs/NEED-CANDIDATE-DISPOSITION.md`.
...
[METHODOLOGY_SCENARIO]
{
  ...
  "steps":[
    {"id":"SCN-02-S0N","title":"Resolve the requested outcome before choosing its future-state host","semanticRefs":["planning/documentation/idtspe-methodology/active/idtspe-core/resolution/needs/NEED-CANDIDATE-DISPOSITION.md"]},
...
**Derived From / Owners:** `planning/documentation/idtspe-methodology/active/idtspe-core/resolution/findings/FINDING-DISPOSITION.md`, `planning/documentation/idtspe-methodology/active/ai-reviewability/AI-OUTPUT-REVIEWABILITY.md`.
...
**AI Action:** Preserve accepted semantic meaning/owner boundaries and continue with the deterministic correction, current semantic completion or bounded local realization choice. Existing Requirement representation may be completed when the missing must-hold is already implied by current authority.
...
**Method / Mechanics:** For `RE-0`, use the uniquely justified correction or complete the missing explicit current Requirement/invariant when accepted owner semantics already entail it; do not manufacture a Proposal just to re-select accepted meaning. For `RE-1`, compare only material local realization alternatives if useful. Ambient Session authorization/proposal-driven policy still governs actual material action when active, but that interaction policy is not itself the semantic action of this Scenario step and formal IDTSPE Proposal State is normally unnecessary.
**Possible Result:** Current semantic completion and/or deterministic/local realization correction ready for the normal representation/realization boundary, with no new semantic Decision.
**Derived From / Owners:** `planning/documentation/idtspe-methodology/active/idtspe-core/resolution/findings/FINDING-DISPOSITION.md`.

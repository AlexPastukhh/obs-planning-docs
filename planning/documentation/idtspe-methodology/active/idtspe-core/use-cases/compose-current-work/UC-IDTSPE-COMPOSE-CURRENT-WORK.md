...
1. Start from the actual USER request/current Work Concern and the current integrated state if one exists. Reuse known facts, accepted Decisions and existing owner results; when an applicable [`Resolution Carry-Forward`](../../resolution/continuation/RESOLUTION-CARRY-FORWARD-PROJECTION.md) exists, scan its compact surviving-state references for orientation and open only the canonical items that are material now.
2. Apply the [`Contextual Methodology Application Contract`](../../runtime/applicability/CONTEXTUAL-METHODOLOGY-APPLICATION-CONTRACT.md). Do not treat IDTSPE always-on status as a reason to instantiate every mechanism.
...
6. Decide whether a bounded Target/result boundary is independently useful. Use [`planning/documentation/idtspe-methodology/active/idtspe-core/runtime/target-work/projections/DYNAMIC-TARGET-FORMATION.explanatory-projection.md`](../../runtime/target-work/projections/DYNAMIC-TARGET-FORMATION.explanatory-projection.md); `no new Target` is a normal outcome.

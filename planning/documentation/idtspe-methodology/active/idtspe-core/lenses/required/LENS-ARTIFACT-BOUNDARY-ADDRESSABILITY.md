...
For Markdown documentation, use an ordinary **relative Markdown link** as the default lightweight navigation relation when direct traversal is materially useful. Prefer a direct link over a bare path/ID/prose mention that forces the reader to search manually; use a stable fragment when the relation is specifically to one section. A valid link does not create ownership/dependency/synchronization authority, and zero broken links does not prove that navigation is semantically useful. Broken targets/fragments and links to stale/non-authoritative destinations are representation defects. Escalate to registries or Linked Notes only when overview/backlink/query/tracking behavior is independently justified.

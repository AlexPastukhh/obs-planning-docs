...
= proportional Core evaluation that determines what the need means,
...
```text
USER / other producer expresses wanted outcome
        │
        ▼
   Need Candidate
        │
...
Need Candidate Disposition
        │
        ├─ ground exact Source/Evidence
        ├─ normalize desired outcome without inventing solution
        ├─ materiality
...
If Need review discovers that accepted current meaning already requires the outcome but implementation/Evidence contradicts it, route that newly established contradiction through Finding Disposition.
...
`idtspe.needs.review` is a reusable orchestration surface over this contract. It does not create a second lifecycle. Canonical methodology scenarios may reference this owner semantically; Planning Helper may derive the command equivalent from those semantic references. Scenario prose must remain command-free.

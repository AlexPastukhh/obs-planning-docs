...
| 09 Consistency / Verification | `UC-IDTSPE-REVIEW-CONSISTENCY`; conditional `TM-TEST-STRATEGY`, optional `TM-TEST-DESIGN`, `TM-PRACTICAL-TEST`, Test Proof Lens review | shared/local proof planning only under real pressure + implemented practical Evidence; straightforward tests may go directly to Exact Realization |
| 10 Pre-Update / Exact Realization | inherited Core `TM-PRE-UPDATE-PLAN` when review-first change planning is useful; inherited Core `TM-EXACT-REALIZATION` for exact directly integrable output | Pre-Update is optional/read-only; Exact Realization is code-first with explicitly authorized integration/verification/repair |
| 11 Evidence / Reconciliation | direct `LENS-TEST-PROOF-EVIDENCE` coverage review + revalidation + frequent Lens refresh | coverage review is a Lens operation, not a separate Target; Core Finding Disposition selects the narrowest real correction/revalidation destination |

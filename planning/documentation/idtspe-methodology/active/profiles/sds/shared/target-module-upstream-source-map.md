...
| `TM-DOMAIN-DISCOVERY` | Scenarios; Scenario DATA; Behavior Items; local/shared must-hold conditions/invariants | Need/solution/Application lineage; Prototype/current implementation Evidence; change-path findings |
...
| `TM-SLICE-STRATEGY` | Scenarios; Scenario DATA; Behavior; local/shared must-hold conditions; Screens; Domain meaning | Need/solution/Application lineage; current implementation; `SDS-WORKSPACE-EVOLUTION.md` + WEUC Lens findings; delivery/dependency constraints |
| `TM-IMPLEMENTATION-SLICE` | selected Useful Vertical Result Definition; one Primary Scenario; DATA/Behavior/must-hold conditions; Domain meaning; Screens for UI/full-stack | Slice Strategy; current implementation; `SDS-WORKSPACE-EVOLUTION.md` + L4/L5/L6 findings/accepted architecture Decisions; Cross-Cutting/delivery constraints |
| `TM-FRONTEND-SLICE` | parent Useful Vertical Result; one Primary Scenario; Screens; Scenario DATA/Behavior; frontend-relevant must-hold conditions | current client code/evidence; `SDS-WORKSPACE-EVOLUTION.md` + L4/L5/L6 findings; design/accessibility/Cross-Cutting constraints |
| `TM-CROSS-CUTTING-CONCERN` | repeated/shared Requirement/responsibility across consumers; affected Slice/implementation owners | current implementation/runtime Evidence; architecture/WEUC Lens findings; quality constraints |
...
→ local evolution/architecture findings
→ optional map update candidate back to TM-WEUC
```

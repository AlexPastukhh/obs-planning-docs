...
This does not create a future Requirement type. `BR-*`, `SR-*`, `IR-*` and `PFR-*` keep their normal family/owner semantics; only their temporal host differs. Family/owner authority remains separate from local discovery/provenance under [`planning/documentation/idtspe-methodology/active/profiles/sds/profile-contracts/semantic-families/SEMANTIC-FAMILY-AUTHORITY-AND-PROVENANCE.md`](../semantic-families/SEMANTIC-FAMILY-AUTHORITY-AND-PROVENANCE.md).
...
Reusable `Requirement Type`, the common Requirement table contract and `QRPE / Examples` semantics are owned by [`planning/documentation/idtspe-methodology/active/profiles/sds/profile-contracts/requirements/REQUIREMENT-CLASSIFICATION-AND-REPRESENTATION.md`](REQUIREMENT-CLASSIFICATION-AND-REPRESENTATION.md). Type/classification never changes natural Requirement ownership or temporal hosting.

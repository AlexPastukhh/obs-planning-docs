...
| [`../registries/LENS-ATTACHMENT-MAP.md`](LENS-ATTACHMENT-MAP.md#sds-lens-attachment-map) | high-level projection of predictable Unit → Lens attachments | review/audit needs to see where Lens are predictably attached across SDS + inherited Core Target Modules | read as projection only; follow the concrete `RU-*` owner for normative attachment and the concrete Lens for triggers | a Unit attachment changes or projection drift is suspected |
| [`../registries/LENS-ATTACHMENT-MAP-INTEGRITY.md`](LENS-ATTACHMENT-MAP-INTEGRITY.md#sds-lens-attachment-projection-integrity) | exact parity audit for Unit-owner attachments ↔ attachment map | projection drift is suspected or Unit/Lens registry/attachment structure changed | derive expected attachments from concrete Core+SDS Unit owners, validate registered Lens identities, compare exact map membership/strength and report PASS/DRIFT | any Unit inventory/attachment, Lens registry membership or map row changes |

...
Session ambient interaction rules are established through [`session/README.md`](session/README.md); Session currently owns no methodology-use UC family. Generic Documentation capabilities are discovered through [`documentation/`](documentation/README.md) and its scoped registry. IDTSPE/SDS and other specialized areas own their own current semantic navigation and are not aggregated here.

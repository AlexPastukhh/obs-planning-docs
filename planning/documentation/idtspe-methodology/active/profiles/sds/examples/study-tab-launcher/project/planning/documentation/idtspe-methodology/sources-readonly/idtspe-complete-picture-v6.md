# IDTSPE Complete Picture v6 — Full Flow, Lens Map, Decision Persistence, WEUC Portfolio and Scenario Validation
...
This file is intended to replace `idtspe-complete-picture-v5.md` as the current complete planning capture. It is operationally tested by `idtspe-sds-end-to-end-scenario-workbook.md` and expanded by `idtspe-decision-persistence-and-weuc-full-picture.md`.
...
# 73. Repository Similarity Is Not Semantic Confirmation

A current repository command/workflow/template may look very close to the selected IDTSPE model.

That is not sufficient.

Before binding any existing mechanism into the future IDTSPE composition, run:
...
↓
TARGET IDTSPE semantics
↓
overlap / missing / duplicate / conflict
↓
...
фулл / модульный / мини SDS profiles
planning-concerns-and-decisions-model
Linked Notes dependency facilities
Pre-Update workflow
```
...
```text
"repository already has X"
```

when the only evidence is:

```text
"repository has something with a similar name/shape"
```

The consistency/deduplication audit is part of future repository realization.

---

# 74. Decision Persistence — Strengthened Invariant

For every material IDTSPE instance, preserve all three selected Decision levels:
...
They define different reusable planning knowledge.

## Target-Scope Decision preserves

```text
why this bounded problem/result was selected
what is outside/deferred
why another Target route was not selected
what should reopen scope
```

## Question-Set Decision preserves

```text
which RQs were selected
which were split/merged/deferred/delegated
why this question set was considered sufficient
what should reopen question discovery
```

## Answer Decisions preserve

```text
which Idea/Variant was accepted
rationale
Sources/Evidence
residual Q/R/P
revalidation hooks
saved alternatives
WEUC impact when relevant
```

A material Target is not durably planned if these choices disappear after the chat turn.

This does not imply one file per Decision.

---

# 75. Decision Storage / Linked Notes Direction

Preferred semantic pattern:

```text
selected meaning
→ integrated into narrowest real Target owner

Decision trace
→ stored beside correct planning/semantic owner when material

Linked Notes / generated projection
→ builds cross-target Decision Portfolio
```

Do not create a manually duplicated global Decision authority merely for overview.

Desired generated views:

```text
decisions(target)
decisions(area)
residualConcerns(decision)
revalidationHooks(decision)
weuc(decision)
supersededDecisions(area)
```

The portfolio is for navigation/revalidation/full-picture analysis.

---

# 76. Full WEUC Picture — Operational Definition

A full WEUC picture is not:

```text
one architecture health score
```

It is:

```text
material current/future Workspace demand
+
contextual WEUC instances
+
expected/current work paths
+
...
Observed WEUC Evidence
+
hot paths / Change Pressure / Change Axes
+
residual revalidation state
```

The full picture should be inspectable both:
...
# 77. WEUC Demand Basis

Every material future WEUC instance should be justifiable.

Ask:

```text
What potential/current Need would cause this Workspace change?
What real useful result would it provide?
What Evidence makes that Need plausible?
How soon?
How valuable/consequential?
Could the Need be satisfied without changing this code/area?
```

A future architecture instance should therefore record, when material:
...
```text
generic future flexibility
→ automatic abstraction
```

---

# 78. WEUC Probability / Timing

Default qualitative model:
...
Do not invent numeric probabilities or weighted scores.

Use numbers only when actual data makes them meaningful.

---

# 79. WEUC Five-Checkpoint Lifecycle

## A — Concept / application-direction checkpoint

Purpose:

```text
discover high-level Workspace demand hypotheses
```

Inputs:

- likely Scenarios;
- Solution/Application responsibility;
- roadmap;
- known external volatility;
- existing workspace history when any.

Output:

```text
WEUC Type candidates
early contextual demand
Need Basis
likelihood/horizon/value/confidence
```

Architecture path precision remains low.

## B — Pre-code concrete realization checkpoint

When Slice/realization owners/seams are concrete:

```text
instantiate material WEUC against planned Workspace areas
trace expected change paths
compare preparation-now vs deferred cost
feed architecture RQs
```

This is the main pre-code architecture checkpoint.

## C — Immediate post-code checkpoint

Re-trace affected WEUC against actual:
...
methods
tests
runtime/diagnosis paths
```

Update path confidence.

## D — Real maintenance/evolution event

Actual workspace change is high-value Evidence.
...
## E — Material portfolio review

Run before/after major architecture changes, after several Slices, repeated incidents, major topology change or explicit review.

---

# 80. Pre-Code WEUC Portfolio Algorithm
...
2. Generate WEUC Type candidates.

3. Instantiate Types against concrete Workspace areas.

4. Apply Need / alternative-route admission gate.

5. Deduplicate equivalent instances.
...
10. Derive supported pressure/Change Axes.

11. Feed only material architecture RQs.
```

---

# 81. Architecture Decision Against WEUC Portfolio

Do not optimize architecture against one attractive future case.

For a material architecture RQ:

```text
1. identify current important Workspace work
2. select driving WEUC instances
3. also select important work that could become worse
4. generate Idea Variants
5. evaluate each Variant across the selected portfolio
6. evaluate Architectural Tax
7. select Decision
8. save Accepted WEUC Impact
9. save residual Q/R/P + revalidation hooks
```

Example:

```text
Idea improves "add provider"
but makes "change shared failure semantics" require duplicate edits
...
# 82. Accepted WEUC Impact

For a material selected Decision, record what the architecture claims.
...
Add provider:
  provider + registry + provider test

Change shared failure semantics:
  shared orchestration only

Diagnose provider failure:
  provider identity visible at shared boundary

Architectural Tax:
  one contract + registry

Reconsider:
  capability-specific logic leaks across shared owners
```

This is a planning expectation.

It is not Observed Evidence.

---

# 83. Observed WEUC Evidence And Reconciliation

After realization / real maintenance:

```text
Accepted WEUC Impact
vs
Observed actual work
```
...
actual proof/diagnosis
unexpected work
actual incidents/friction
```
...
Do not overwrite provenance of the earlier expectation.

---

# 84. Representative WEUC Experiment

When a future change is important enough to test architecture but has not happened yet, a bounded representative exercise can provide Evidence.
...
which files/classes/methods change
which tests change
...
It is not a production Scenario merely because it exercises the architecture.

---

# 85. Full WEUC Portfolio Views
...
```text
which Decisions shape many important WEUC?
...
# 86. Scenario Acceptance Test Suite

The separate workbook:

```text
idtspe-sds-end-to-end-scenario-workbook.md
```

contains four operational scenarios.

## Scenario A — Greenfield Research Capture

Tests:

```text
Need → Solution → Responsibility → Scenario → Domain → Slice
→ pre-code WEUC
→ Architecture Decision
→ test plan
→ realization
...
## Scenario B — Add Zotero to existing app

Tests:

```text
low-level integration Trigger
→ targeted reverse justification
→ reuse existing Scenario
→ prior Risk fires
→ architecture RQ reopens
→ new Slice
→ post-code WEUC confirmation
```

## Scenario C — Real maintenance shows architecture is too expensive

Tests:

```text
Practical Evidence as Trigger
→ WEUC portfolio
→ architecture replan
→ representative WEUC experiment
→ state reconciliation
```

## Scenario D — Correct result is "do not build"

Tests:

```text
Need/Value/Scope rejects attractive feature
→ no Slice required
```

These scenarios are methodology acceptance tests, not repository/product truth.

---

# 87. Tampermonkey Implications From The Scenarios

Current command palette infrastructure is useful, but target semantics require consistency review before reuse.

Minimal future generic UX suggested by the scenarios:

```text
1. generic target-planning entry
2. explicit Decision revalidation
3. WEUC portfolio/full-picture review
4. optional planning-conformance diagnostic
```

Target-specific commands may remain shortcuts.

Validators should normally run automatically rather than require user buttons.

Tampermonkey remains:

```text
projection / invocation UI
≠ semantic authority
```

---

# 88. Current-Repo Reuse Gate

Before modifying current commands/workflows to implement the selected model, inspect for every candidate:
...
expected output
permission
current Q/R/P/Decision semantics
current WEUC/dependency responsibilities
```
...
This gate is mandatory because current naming similarity is not proof of semantic equivalence.

---

# 89. Current Complete Picture — Final Operational Formula
...
→ Target-Scope Decision
→ typed Sources
→ prior-Decision challenge scan
→ Question-Set Decision
→ applicable Lenses
→ Ideas
→ Q/R/P + Evidence
→ Answer Decisions
→ residual revalidation hooks
→ cross-Decision composition review
→ Target Decision State
→ complete Target projection
→ conformance validation

WORKSPACE / ARCHITECTURE
→ WEUC demand hypotheses
→ concrete pre-code instances
→ portfolio architecture evaluation
→ Accepted WEUC Impact

DELIVERY
→ explicit Pre-Update
→ separately authorized realization
...
PERSISTENCE / LINKED NOTES
→ current Target
→ three Decision types
→ residual Q/R/P
→ Evidence
→ Decision ↔ WEUC portfolio
→ trusted Sources for next planning loop
```

...
- [`use-cases/`](use-cases/README.md) — current Documentation Use-Case owners, including the fundamental methodology applicability Use Case.
- [`processes/`](processes/README.md) — reusable Documentation Processes reached from Use Cases; they do not replace Use-Case Results or semantic owners.
- [`templates/`](templates/README.md) — shape-only templates introduced by the fundamental layer.
...
Exact pre-fundamental `README.md` and `use-case-registry.md` snapshots remain under [`legacy/`](legacy/README.md) as provenance. They are not current semantic owners, and relative links inside them may reflect their former canonical location.

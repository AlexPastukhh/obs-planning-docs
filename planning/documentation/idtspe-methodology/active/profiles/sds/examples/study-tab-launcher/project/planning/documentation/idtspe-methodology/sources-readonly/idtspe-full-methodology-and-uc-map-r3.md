...
# Revision — Modular IDTSPE: Lenses, Scope, Resolution Questions And Planning Topology

## Sources, Lenses, Patterns And Contracts

A concrete IDTSPE instance should be assembled compositionally rather than from one hard-coded checklist.

```text
Source
= what is known/trusted/observed

Lens
= how a candidate scope/question/idea/decision should be challenged

Pattern / Idea Archetype
= a reusable candidate answer

Target Contract / Template
= what a valid reusable Target result must contain

Planning Topology
= how a larger problem is decomposed into staged Targets whose outputs become Sources of Truth
```

Concrete Q/R/P are not Lenses. A Lens defines evaluation prompts; the resulting concrete concern belongs to the object actually being challenged.

## Lens Selection

Lens selection is proportional and can come from:

```text
core/default lenses
+ Target-family preset
+ Target Contract
+ current Sources / Evidence / Change Pressure
+ explicit user/agent selection
```

A Lens should eventually define an applicability condition. Potential generic lenses include Need/Scope Validity, Source-of-Truth/Reuse and Evidence/Reversibility. Contextual lenses include WEUC, Dependency/Reuse, Architecture Pressure, File/Artifact Impact, security, performance and operations.

WEUC has several distinct roles:

```text
Current WEUC State -> Source of Truth
WEUC Lens -> evaluation methodology
Projected WEUC Impact -> candidate/Decision analysis
Observed WEUC Evidence -> later reconsideration evidence
```

Architecture documentation can accumulate reusable Lenses, Patterns, known risky situations, typical Q/R/P prompts and applicability/failure evidence.

## Target / Scope Discovery Comes Before Question Discovery

The first bounded planning choice is not the answer to a design question. It is the planning scope itself.

```text
Need / Trigger
+ current Planning Topology when any
+ available Sources
↓
Target/Scope Candidates
↓
Scope Q/R/P through relevant lenses
↓
Target-Scope Decision
↓
Selected Target Result / Scope
```

Target Scope includes:

- the concrete problem/goal for this stage;
- desired Target Result;
- boundaries/non-goals/deferred meaning;
- Target Type / Contract;
- trusted upstream Sources;
- intended output that can become a Source of Truth for later IDTSPE instances.

Target-Scope Q/R/P challenge whether this is the right bounded problem/result. Residual concerns move to the Target-Scope Decision and define when scope should be reopened.

## Working Rename: Decision Question -> Resolution Question

The former `Decision Question` is better understood as a question used to discover candidate answers before a Decision exists.

Working term: **Resolution Question (RQ)**.

```text
Resolution Question
= a scoped question whose candidate answers are Ideas/Variants
  and whose accepted answer becomes a Decision
```

The final public name remains open.

After Target-Scope Decision:

```text
Selected Target Scope
+ Target Contract
+ Sources of Truth
+ reusable RQ archetypes/presets
+ current concerns/evidence
↓
...
Thus two different choices exist:

```text
1. What bounded problem/result are we solving now?
2. Which questions must be answered to construct that result well?
```

## Shared Evaluation Concern Set -> Lens Set

The earlier `Shared Evaluation Concern Set` is generalized as a selected Lens Set for an active RQ.

```text
Active RQ
→ selected Lenses
→ evaluation prompts / reusable concern archetypes
→ instantiate concrete Q/R/P on each candidate Idea
```

Example:

```text
RQ: How should Service X integration be realized?
...
Each Idea receives the applicable shared lens checks plus its own Idea-specific Q/R/P.

## File / Artifact Impact Lens Versus Pre-Update

The File/Artifact Impact Lens is used while comparing Ideas/Decisions. It asks how broad the owner/file/dependency/projection burden of each candidate will be.

Pre-Update remains a post-decision adapter that converts the accepted Target State into the exact dependency-aware transition plan.

```text
Lens -> improves Decision quality
Pre-Update -> plans realization of the selected Decision State
```

## Planning Topology

For complex multi-stage problems, it is useful to design a rough problem-solving map before executing every stage.
...
The goal is to maximize one-directional planning, isolate concerns by stage, resolve expensive uncertainty early, produce reusable Sources of Truth and minimize later invalidation.

A Planning Topology can itself be the Target of an IDTSPE instance when no suitable preset exists.

## SDS Position

SDS is best understood as a rich pre-designed Application Planning Topology / Target-family preset.

It provides reusable:

- Target Types;
- Target Contracts/templates;
- rough directional ordering;
- Source dependencies;
- RQ archetypes;
- default Lens sets;
- handoffs;
...
It does not pre-create concrete Needs, Scenarios, Domains, Slices, Decisions or Q/R/P. Those are unique Target instances.

Even with SDS, concrete Target/Scope and RQ discovery remains proportional: presets reduce discovery cost but do not eliminate reasoning.

## One-Off Problems

A smaller problem may not justify a reusable Planning Topology. It can still run:
...
If the problem becomes large/recurrent, IDTSPE can itself decide that a reusable topology, Target Type or Lens preset is worth establishing.

## Fixed, Configurable And Dynamic Parts

Fixed reusable IDTSPE mechanics:

- Need/root-justification handling;
- Target/Scope discovery;
- Source-of-Truth reuse;
- Lens selection;
- Resolution Question discovery;
- Idea/Q/R/P/Decision lifecycle;
- Target projection;
- Evidence/reconsideration loop;
- dependency-aware Pre-Update boundary.
...
## Consolidated Generic IDTSPE

```text
TRIGGER
↓
resolve fundamental NEED
↓
select/reuse/design PLANNING TOPOLOGY when needed
↓
TARGET/SCOPE DISCOVERY
↓
TARGET-SCOPE DECISION
...
for each ACTIVE RQ:
  ...
  instantiate shared-lens concerns
  derive Idea-specific Q/R/P
  collect/use EVIDENCE
  review WEUC/dependency/etc impacts
  select DECISION
...
REALIZATION
↓
PRACTICAL / WEUC / OTHER EVIDENCE
↓
review Decision / Question-Set / Target-Scope residual Q/R/P
↓
reopen the narrowest invalid level
↓
loop
```
...
- **IDTSPE** — generic closed-loop planning/evolution engine.
- **SDS** — Application-specific rich Planning Topology / Target-family preset.
- **WEUC** — workspace-evolution Source system + Lens + Impact/Evidence feedback loop.
- **Architecture Planning** — owner of architecture-specific Lenses, Patterns, paths/pressure analysis and architecture Decision support.
- **Linked Notes** — potential resolver/query/projection layer for Sources, dependencies, Evidence and freshness; not semantic owner.
- **Dependency methodology** — reusable Lens + relation/source model for ownership/reuse/impact.
- **Pre-Update** — post-decision realization-planning adapter.
- **Q/R/P** — concrete challenge/evaluation/monitoring concerns generated at lifecycle owners, often through reusable Lenses.
- **Ideas** — candidate answers to active Resolution Questions.
- **Decisions** — accepted answers with residual Q/R/P and reconsideration contracts.
- **Target** — selected reusable result for one scoped IDTSPE instance, projected through its Target Contract/template.

## Open Questions

- Final name for `Resolution Question`.
- Minimal canonical Lens contract.
- Whether Lens discovery needs a catalog/index or only owner-local definitions + generated discovery.
- Canonical representation of Planning Topology.
- How presets reference Target Contracts, Lenses, RQ archetypes and Pattern libraries without duplicate authority.

## Current Direction Decisions

- Treat Shared Evaluation Concern Sets as reusable Lens Sets.
- Build IDTSPE instances compositionally rather than with one hard-coded universal checklist.
- Target/Scope Discovery precedes Resolution Question Discovery.
- For complex multi-stage problems, use or design Planning Topology to maximize one-directional planning.
- Treat SDS as a rich pre-designed Application Planning Topology/Target-family preset.
- Presets reduce discovery cost but do not eliminate proportional concrete Target/Scope/RQ validation.
- Use `Resolution Question` as the current working name; final terminology remains open.

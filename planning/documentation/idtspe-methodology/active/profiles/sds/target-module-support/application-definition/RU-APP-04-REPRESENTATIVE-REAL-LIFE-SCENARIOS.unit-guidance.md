...
Which AB-* items manifest/close at that point, if useful to mark? If only one owned boundary/constraint clause is evidenced, would an `AB-* / BC-*` reference be more truthful?
...
Many-to-many relations are normal: one Benefit may appear in several RLS and one RLS may demonstrate several Benefits. The RLS references Benefits; it does not own them. A plain `AB-*` reference remains valid for whole-Benefit relevance; when only one addressable owned boundary/constraint clause is involved, `AB-* / BC-*` may be used for precision without implying whole-Benefit realization.

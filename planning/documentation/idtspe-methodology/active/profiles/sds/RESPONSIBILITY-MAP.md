...
| SDS Unit→Lens predictable-attachment projection parity / audit | [`registries/LENS-ATTACHMENT-MAP-INTEGRITY.md`](registries/LENS-ATTACHMENT-MAP-INTEGRITY.md#sds-lens-attachment-projection-integrity) — `SDS.LENS-ATTACHMENT-PROJECTION-INTEGRITY` | Audit method only; concrete Unit owners remain normative and `LENS-ATTACHMENT-MAP` remains projection-only |
...
| SDS exact codebase realization / integration | [`target-modules/TM-CODE-REALIZATION.md`](target-modules/TM-CODE-REALIZATION.md#tm-code-realization) — `TM-CODE-REALIZATION` | Owns literal source/test/codebase implementation under SDS; broad non-code exact realization remains Core `TM-EXACT-REALIZATION` |

...
## Entry 038 — Assistant — discussion result

Да, тут было несколько смешений, и после твоего уточнения модель можно упростить.

1. `Uncertainty / Assumption / Reversibility` — это planning Lens **до/во время выбора**. Она спрашивает: что мы не знаем, какие assumptions делаем, стоит ли добыть Evidence до Decision, насколько решение обратимо и сколько стоит ошибиться.

2. `Revalidation` — не Lens. Это **поздний lifecycle-механизм** над уже принятым Decision. Он берёт residual Decision Q/R/P + новые Evidence/Decisions/Sources/constraints и решает: reaffirm или reopen answer / RQ set / Target Scope.

3. То, о чём ты говоришь как о подготовке будущих признаков и вопросов, лучше всего называется `Decision Revalidation Readiness Validator`. Это проверка корректности выполнения IDTSPE: если после Decision остаются material Q/R/P, они должны быть превращены в usable reconsideration contract.

Для residual `Q` validator проверяет: что неизвестно, почему важно, какое Evidence/когда может дать ответ и какие ответы confirm/weaken/reopen/invalidate Decision.

Для residual `R`: risk hypothesis, leading indicators, Evidence source, horizon, threshold/event, что reopen, fallback Idea.

Для residual `P`: известная текущая проблема, почему она пока приемлема, как измеряется, где tolerance threshold/deadline и что делать после него.

Если residual Q/R/P нет, validator не заставляет их выдумывать.

`Guard` здесь хуже: Guard нужен для жёсткого запрета вроде `plan-only → no mutation`. Здесь нужна содержательная проверка полноты reconsideration hooks, поэтому Validator точнее; его failure при material Decision можно сделать blocking.

4. `Proof / Verification / Operability` тоже было смешано. Peer Lens лучше назвать `Verifiability / Observability / Operability`: можно ли candidate проверить, наблюдать, диагностировать и эксплуатировать разумной ценой. Он может породить Q/R/P и proof/evidence plan. А выполненный test/proof/observation — уже Evidence.

5. Evidence, criticism и arguments:
- Evidence = исходный факт/наблюдение/авторитетный Source;
- material negative interpretation обычно становится Q/R/P;
- positive/negative comparative reasoning между Ideas — evaluation finding/rationale;
- выбранная интерпретация сохраняется как Decision rationale.
Отдельную first-class сущность `Argument` пока вводить не надо.

6. `Constraint / Delivery Lens` лучше убрать. Deadline/capacity/milestone/release/external dependency уже `DELIVERY_CONSTRAINT` Sources. Они влияют на Scope/RQ/Idea/staging через обычные Rules. Отдельная peer Lens пока не нужна.

7. Чтобы не раздувать taxonomy, публичные reusable категории теперь минимальные:
`Lens`, `Validator`, `Guard`, `Rule`, `Pack`.
Все transition/persistence/derivation/reconciliation особенности — просто виды обычных Rules в prose/metadata, а не новые first-class типы.

Обновлены:
- `idtspe-coherent-model-v4.md`
- `idtspe-evidence-revalidation-verification-clarification.md`
- `idtspe-discussion-history-r7.md`

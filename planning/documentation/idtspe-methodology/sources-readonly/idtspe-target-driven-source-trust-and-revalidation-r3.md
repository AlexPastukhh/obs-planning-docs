...
## 23. Revision — Revalidation At Question-Scope And Answer Levels

Selective revalidation has two explicit levels.

### Answer-level revalidation

```text
Decision residual Q/R/P
+ new Evidence / WEUC impact
→ reopen the Decision Question
→ reassess candidate answers
```
...
+ new Evidence / changed Sources
→ reopen Decision Question Discovery
→ add/remove/split/merge/reframe Decision Questions
```

The engine should distinguish:

```text
Did the new challenge undermine the answer to a known question?
```

from:

```text
Did it undermine the assumption that we are asking the right question(s)?
```

Only the corresponding level is reopened.

## Revision — Revalidation Across Scope, Questions And Answers

Selective revalidation now has three explicit levels:

```text
Target-Scope Decision Q/R/P
→ reopen whether we are solving the right bounded problem/result

Question-Set Decision Q/R/P
→ reopen which Resolution Questions should be asked

Decision Q/R/P
→ reopen the accepted answer to one Resolution Question
```

Evidence should reopen the narrowest level it actually invalidates.

Reusable Lenses help identify the relevant challenge, but concrete Q/R/P belongs to the challenged scope/question/Idea/Decision.

A pre-designed Planning Topology/SDS preset reduces how often scope must be invented from scratch, but it does not make concrete scope decisions immune to new evidence.

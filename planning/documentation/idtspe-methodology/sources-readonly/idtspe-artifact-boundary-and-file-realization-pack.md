...
Status: selected conceptual extension to IDTSPE/documentation methodology  
Purpose: decide **what should become a separate artifact/file in a concrete case**, independently from semantic planning, and then validate the resulting repository representation.  
...
Therefore file work is an independently composable pack:
...
Full SDS default:
  ...
  Slice stable owner
  contextual WEUC register when durable instances justify it
...
+
generated Linked Notes Decision Portfolio
```

The generated portfolio is a separate artifact because:

```text
different generation lifecycle
independent navigation/query role
...
Suppose only two small contextual WEUC findings exist and both are local to one architecture decision.

Artifact Discovery may select:

```text
keep them beside the Architecture Decision
```

Suppose later:

```text
15 durable contextual WEUC instances
several architecture Decisions reference them
portfolio review is frequent
Linked Notes queries need stable IDs
```

Then:

```text
project-local WEUC Instance Register
```

becomes justified.

This is exactly why preset alone is insufficient.
...
Artifact Boundary Discovery may itself use the WEUC Lens.
...
5. Use WEUC / Dependency lenses when material
...
WEUC methodology
→ defines WEUC instance/register forms
...
```text
artifactOwners(target)
artifacts(target)
representation(target)
duplicateOwners(area)
generatedProjections(area)
```

support validation without becoming semantic authority?

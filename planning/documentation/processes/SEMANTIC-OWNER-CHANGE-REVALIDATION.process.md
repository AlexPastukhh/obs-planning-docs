...
4. Find tracked `Semantic Owner Dependency` consumers of the affected responsibility/owner and classify them by `RESTATES`, `CONTEXTUALIZES`, `EXTENDS`, `REPRESENTS`, or `MIGRATES`. For a material TM/Lens change, inspect the current owners in scope for declared inbound dependencies; an on-demand dependency audit may assemble a reverse lookup from those declarations. Ordinary Markdown links remain navigation and are not dependency edges.
5. When the changed owner is an active Lens, Target Module, or Module-defined Unit, inspect affected concrete Unit `Lens Attachments` blocks and Lens owners. An on-demand attachment audit may assemble a bounded table for the current scope. Context-emergent applicability still requires Lens registry discovery and concrete applicability checks.

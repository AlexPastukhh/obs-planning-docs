...
|---|---|---|---|---|---|---|
| `давай архив`, `собери архив`, `give arch`, `replacement package` | `build replacement archive` | Produce a replacement ZIP plus short OBS-ACTION handoff; producer-only, not archive read-source/local apply mode. | Use exact checked source. Earlier archives are not current automatically; an explicitly provided/selected snapshot may be used after repository/target/coverage verification. Otherwise use fully readable current repository files; request only minimum missing exact source and never guess touched base. | Targeted/full depending on touched files and source certainty. | Project root UCM, delegated build-replacement command, producer workflow and shared package protocol owner when present. | One full replacement ZIP + OBS-ACTION. Complete base/replacement payloads. No generated local apply/diff/finalization commands. |
| `давай архив с review diff file`, `give arch rev dif`, `archive with review diff file` | `build archive with review diff` | Produce a legacy reviewable replacement archive and repository-stored review diff when explicitly requested. | Use only when that legacy transfer is approved; source selection, exact local-base verification, apply/diff and review come from this route’s own ownerFiles rather than inheriting the producer-only `давай архив` runtime. | Targeted/full depending on touched files. | Project root UCM, explicit legacy reviewable-output/review-diff/application owners and target files. | Route-owned archive + review-diff flow; no commit/push before its required review gate. |

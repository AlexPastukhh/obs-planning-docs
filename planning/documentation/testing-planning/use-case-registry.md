...
Current routing: generic proof evaluation is owned by `LENS-TEST-PROOF-EVIDENCE`; methodology-integrity guard maintenance is owned by `UC-IDTSPE-MAINTAIN-METHODOLOGY-INTEGRITY-TESTS`; literal realization uses the applicable Exact Realization owner; real implemented-subject Evidence uses `TM-PRACTICAL-TEST` when warranted.
...
All four UCs may use [`templates/TESTING-PLAN-TEMPLATE.md`](templates/TESTING-PLAN-TEMPLATE.md) proportionally. The template is a representation surface, not a fifth Testing Use Case.

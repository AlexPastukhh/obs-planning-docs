...
Declared methodology dependencies / Unit Lens Attachments examined, when applicable
...
Examples/product checks/manual audits requiring review
```
...
12. When the planned change involves active IDTSPE Target Modules, Lenses, Module-defined Units, their dependency declarations, attachment declarations, owner paths/anchors or registry identities, inspect the current concrete owner dependency declarations and Unit Lens Attachments in the affected scope. For broader impact discovery, request an on-demand audit table and follow each row to its source. Continue registry-driven Lens discovery for context-emergent applicability; no standing dependency/attachment map is maintained.

...
20. When review scope includes active IDTSPE Target Module/Lens/Module-defined Unit integration, inspect the relevant concrete `Semantic Owner Dependency` and Unit `Lens Attachments` declarations. An on-demand audit table may help enumerate a bounded scope, but each row must lead to its source. Registry-driven discovery remains necessary for context-emergent Lens applicability.

...
- `chatContextToken` is optional and is emitted only when the active command invocation explicitly carried `capture-chat-context`. It is an opaque one-invocation **bind/rebind authority**: successful unique resolution means “make the captured conversation this ChangeSet's persisted Review chat”, whether the ChangeSet was unbound, already bound there or bound elsewhere. It never identifies the package, Repository Target or ChangeSet, never grants repository-operation authority, and MUST NOT be copied into a later `OBS-ACTION` unless that later command invocation supplies its own token. The consumer validates it as a UUID and one token may be associated with only one package/ChangeSet request;
- `chatTabTitle` remains optional legacy fallback metadata. When `chatContextToken` is present, token resolution is authoritative for that action and title matching/rebind preparation is not performed. Legacy title-driven rebind still uses its existing explicit keep/rebind/cancel authorization; token-driven rebind needs no second confirmation because the explicit Helper `Bind + ...` invocation already supplied that authority. When no token is present, existing `chatTabTitle` behavior remains unchanged. Consumer title-matching configuration is local application state and is never carried in `OBS-ACTION`;
...
- token-assisted resolution is asynchronous and begins when an authorized Apply executes. The Java bridge exposes the pending token to the extension; background asks all live ChatGPT tab agents, and an agent answers only from that tab's `sessionStorage` capture record `{chatContextToken, conversationKey, observedTitle, capturedAt}`. Multiple answers for the same `conversationKey` are one logical result; one token reported for different conversation keys is a capture conflict and never guesses a destination;
- successful unique token resolution immediately persists the captured conversation as the ChangeSet Review chat: missing binding → bind, same binding → confirm/refresh, different binding → rebind. This token outcome is independent of repository Apply success/failure. Safely cancellable older Pending/Claimed Review tasks are cancelled when the destination changes; an already `Preparing`/`SendArmed`/`SendClicked` task keeps its frozen original destination and is not rewritten by the new binding;
- repository Apply never waits for token lookup. At the successful Apply/current-ReviewDiff cutoff, a token already resolved to its persisted binding queues that current ReviewDiff to the bound/rebound destination. If lookup is still pending or conflicted, Apply remains successful but that Apply's ReviewDiff is not auto-queued. A later successful resolution still binds/rebinds immediately for future deliveries, but MUST NOT retroactively send the already-skipped ReviewDiff; failed repository Apply likewise does not cancel token lookup or undo a resolved bind/rebind;

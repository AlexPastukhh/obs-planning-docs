# Historical Documentation Workbench Planning Draft

Status: historical migration/provenance source; not a current semantic owner

Historical-content boundary: the body below preserves former-current planning wording for provenance. Terms such as `current Planning Draft`, `selected current behavior` or former routes inside that preserved body do not describe the current repository architecture.
...
| Previous route | `full-picture.md` (removed compatibility view; history preserved here) compatibility pointer |

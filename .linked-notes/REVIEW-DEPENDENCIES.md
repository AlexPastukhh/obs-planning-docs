...
Review Dependencies do not replace Reference Objects. Reference Object freshness remains literal-content based: a materialized `obs-ref:use` is stale when its content differs from the canonical `obs-ref:def` content. Do not add Review Dependency tracking merely to duplicate an existing Reference Object synchronization relation unless there is an additional independent semantic dependency.
